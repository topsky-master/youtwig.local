...
if (isset($_REQUEST['PAYER_NAME']) && isset($_REQUEST['PRODUCT_ID']) && isset($_REQUEST['PAYER_PHONE']) && CModule::IncludeModule("catalog") && CModule::IncludeModule("sale") && canYouBuy($_REQUEST['PRODUCT_ID'])) {
    ...
    $arUserResult = array(
        ...
        "EVENT_NAME" => "SEND_ONE_CLICK_ORDER",
        "PAYED" => "N",
        "CANCELED" => "N",
        "STATUS_ID" => "N"
    ...
    $USER_EMAIL = isset($_REQUEST["PAYER_EMAIL"]) ? trim($_REQUEST["PAYER_EMAIL"]) : 'i@youtwig.ru';
        ...
        if(!empty($productPrice)
            ...
            if (!$bFound && !empty($arUserResult["DELIVERY_ID"]) && mb_strpos($arUserResult["DELIVERY_ID"], ":") === false)
                ...
                foreach ($arDeliveryAll as $arDelivery) {
                    if (
                        isset($arP2D[$arUserResult["PAY_SYSTEM_ID"]]) && 
                        is_array($arP2D[$arUserResult["PAY_SYSTEM_ID"]]) &&
                        (count($arP2D[$arUserResult["PAY_SYSTEM_ID"]]) <= 0 || in_array($arDelivery["ID"], $arP2D[$arUserResult["PAY_SYSTEM_ID"]]))
                    ) {
            ...
            if ($ORDER_ID > 0) {
                ...
                if (Add2BasketByProductIDOld($product_id, 1, array(
                    ...
                    $arEventFields = Array(
                        ...
                        "ORDER_USER" => ((mb_strlen($arUserResult["PAYER_NAME"]) > 0) ? $arUserResult["PAYER_NAME"] : $USER_CLIENT),
                        "PRICE" => SaleFormatCurrency($productPrice['RESULT_PRICE']['BASE_PRICE'] + (!empty($arFields['PRICE_DELIVERY']) ? $arFields['PRICE_DELIVERY'] : 0), $productPrice['RESULT_PRICE']['CURRENCY']),
                        "BCC" => COption::GetOptionString("sale", "order_email", "order@" . IMPEL_SERVER_NAME),
                        "EMAIL" => (mb_strlen($NEW_EMAIL) > 0 ? $NEW_EMAIL : $USER_EMAIL),
                        "ORDER_LIST" => $strOrderList,
                        "SALE_EMAIL" => COption::GetOptionString("sale", "order_email", "order@" . IMPEL_SERVER_NAME),
                        "DELIVERY_PRICE" => SaleFormatCurrency((!empty($arFields['PRICE_DELIVERY']) ? $arFields['PRICE_DELIVERY'] : 0), $productPrice['RESULT_PRICE']['CURRENCY'])
                    );
                    ...
                    $protocol = IMPEL_PROTOCOL;

                    $eventName  = "UNSUCCESS_ORDER";
                    $product_id = intval($_REQUEST['PRODUCT_ID']);

                    $arEventFields = Array(
                        "ORDER_ID" => $ORDER_ID,
                        "ORDER_LINK" => $protocol . IMPEL_SERVER_NAME . '/bitrix/admin/sale_order_view.php?ID=' . $ORDER_ID . '&lang=ru&filter=Y&set_filter=Y',
                        "PRODUCT_LINK" => $protocol . IMPEL_SERVER_NAME . '/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=11&type=catalog&ID=' . $product_id . '&lang=ru&WF=Y'
                    );



                    if ($buy_id != $product_id) {
                        $arEventFields["BASE_LINK"] = $protocol . IMPEL_SERVER_NAME . '/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=16&type=catalog&ID=' . $buy_id . '&lang=ru&WF=Y';
                    }
                    ...
                    CEventLog::Add(array(
                        "SEVERITY" => "WARNING",
                        "AUDIT_TYPE_ID" => "BYONECLICK_TYPE",
                        "MODULE_ID" => "sale",
                        "ITEM_ID" => $ORDER_ID,
                        "DESCRIPTION" => "Не удалось добавить в заказ в один клик товар #" . $product_id

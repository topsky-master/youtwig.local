...
<table class="table table-striped table-bordered table-hover table-support-ticket">
    ...
    <tr>
        ...
        <td>
            <input accesskey="b" type="button" value="<?=GetMessage("SUP_B")?>" onClick="insert_tag('B', document.forms['support_edit'].elements['MESSAGE'])"  name="B" id="B" title="<? echo GetMessage("SUP_B_ALT"); ?>" class="btn btn-primary" />
            <input accesskey="i" type="button" value="<?=GetMessage("SUP_I")?>" onClick="insert_tag('I', document.forms['support_edit'].elements['MESSAGE'])" name="I" id="I" title="<? echo GetMessage("SUP_I_ALT"); ?>" class="btn btn-primary" />
            <input accesskey="u" type="button" value="<?=GetMessage("SUP_U")?>" onClick="insert_tag('U', document.forms['support_edit'].elements['MESSAGE'])" name="U" id="U" title="<? echo GetMessage("SUP_U_ALT"); ?>" class="btn btn-primary" />
            <input accesskey="q" type="button" value="<?=GetMessage("SUP_QUOTE")?>" onClick="insert_tag('QUOTE', document.forms['support_edit'].elements['MESSAGE'])" name="QUOTE" id="QUOTE" title="<? echo GetMessage("SUP_QUOTE_ALT"); ?>" class="btn btn-primary" />
            <input accesskey="c" type="button" value="<?=GetMessage("SUP_CODE")?>" onClick="insert_tag('CODE', document.forms['support_edit'].elements['MESSAGE'])" name="CODE" id="CODE" title="<? echo GetMessage("SUP_CODE_ALT"); ?>" class="btn btn-primary" />
            <?if (LANG == "ru"):?>
            <input accesskey="t" type="button" accesskey="t" value="<?=GetMessage("SUP_TRANSLIT")?>" onClick="translit(document.forms['support_edit'].elements['MESSAGE'])" name="TRANSLIT" id="TRANSLIT" title="<? echo GetMessage("SUP_TRANSLIT_ALT"); ?>" class="btn btn-primary" />
            <?endif?>
        </td>
    </tr>
    ...
    <tr>
        <td></td>
        <td><textarea name="MESSAGE" class="form-control" id="MESSAGE" rows="20" cols="45" wrap="virtual"><?=htmlspecialcharsbx($_REQUEST["MESSAGE"])?></textarea></td>
    </tr>
    ...
    <tr>
        ...
        <td>
            <?
            if (empty($arResult["TICKET"]) || mb_strlen($arResult["ERROR_MESSAGE"]) > 0 )
            {
                if (mb_strlen($arResult["DICTIONARY"]["CRITICALITY_DEFAULT"]) > 0 && mb_strlen($arResult["ERROR_MESSAGE"]) <= 0)
            ...
            ?>
            <select name="CRITICALITY_ID" id="CRITICALITY_ID" class="select-field selectpicker">
                <option value="">&nbsp;</option>
    ...
    <tr>
        ...
        <td>
            <?
            if (mb_strlen($arResult["DICTIONARY"]["CATEGORY_DEFAULT"]) > 0 && mb_strlen($arResult["ERROR_MESSAGE"]) <= 0)
                $category = $arResult["DICTIONARY"]["CATEGORY_DEFAULT"];
            else
                $category = htmlspecialcharsbx($_REQUEST["CATEGORY_ID"]);
            ?>
            <select name="CATEGORY_ID" id="CATEGORY_ID" class="select-field selectpicker">
                <option value="">&nbsp;</option>
    ...
    <tr>
        ...
        <td>
            <?$mark = (mb_strlen($arResult["ERROR_MESSAGE"]) > 0 ? htmlspecialcharsbx($_REQUEST["MARK_ID"]) : $arResult["TICKET"]["MARK_ID"]);?>
            <select name="MARK_ID" id="MARK_ID" class="select-field selectpicker">
                <option value="">&nbsp;</option>
...
<br />
<input type="submit" name="save" value="<?=GetMessage("SUP_SAVE")?>" class="btn btn-success" />&nbsp;

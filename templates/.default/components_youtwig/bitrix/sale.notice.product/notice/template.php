<?php $APPLICATION->IncludeComponent("bitrix:sale.notice.product", "notice", array(
											"NOTIFY_ID" => $arResult["BUY_ID"],
...
<?if ($arResult["STATUS"] == "Y"):?>
	<span id="url_notify_<?=$arParams['NOTIFY_ID']?>" class="url_notify_<?=$arParams['NOTIFY_ID']?>"><?=GetMessage('MAIN_NOTIFY_MESSAGE')?></span>
<?elseif ($arResult["STATUS"] == "N"):?>
	<span id="url_notify_<?=$arParams['NOTIFY_ID']?>" class="url_notify_<?=$arParams['NOTIFY_ID']?>"><a href="javascript:void(0)" onClick="notifyProduct('<?=$arResult["NOTIFY_URL"]?>', <?=$arParams['NOTIFY_ID']?>);"><?=GetMessage('NOTIFY');?></a></span>
<?elseif ($arResult["STATUS"] == "R"):?>
	<span id="url_notify_<?=$arParams['NOTIFY_ID']?>" class="url_notify_<?=$arParams['NOTIFY_ID']?>">
		<a href="javascript:void(0)" onClick="showNotify(<?=$arParams['NOTIFY_ID']?>,this)" id="notify_product_<?=$arParams['NOTIFY_ID']?>"><?=GetMessage('NOTIFY');?></a>
	</span>
<?endif;?>
		...
		<?if($arResult["CAPTCHA_CODE"]):?>
			<tr>
				...
				<span id="popup_captcha_img">
					<img src="/bitrix/tools/captcha.php?captcha_sid=<?echo $arResult["CAPTCHA_CODE"]?>" width="180" height="40" alt="CAPTCHA" /></span></td>
		...
		new BX.PopupWindowButton({
			...
			events : {
				click : function() {
                    ...
                    if($('input[name="user_sessid"]').length) sessid = $('input[name="user_sessid"]').val();
					...
					if (error == 'N')
					{
						...
						BX.ajax.post('/bitrix/components/bitrix/sale.notice.product/ajax.php', data, function(res) {
							...
							{
								notifyProduct(BX('popup_notify_url').value, inNotifyId);
								wind.close();
							}
						});
					}
...
function showNotify(id,element)
{
	inNotifyId = id;

	wind.setBindElement(element);
	wind.show();
	...
	BX.ajax({
        ...
        onsuccess: function(data){
    		...
    		if (BX('url_notify_'+id)){
    			$('.url_notify_'+id).each(function(){
					this.innerHTML = '<?=GetMessage("MAIN_NOTIFY_MESSAGE");?>';
        		});
        	};
        },
        onfailure: function(){

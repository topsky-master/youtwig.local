...
if (!empty($arResult['ITEMS']) && (($arParams['hasError404'] != 'Y')))
{
    ...
    <div id="pos-new-products" class="bx_catalog_list_home new-products catalog clearfix <? echo $templateData['TEMPLATE_CLASS']; ?>">
            ...
            <div class="mozaic clearfix" id="mozaic">
                <div class="row">
                    ...
                    foreach ($arResult['ITEMS'] as $key => $arItem){
                        ...
                        unset($arItem['DISPLAY_PROPERTIES']['QUALITY']['VALUE'], $arItem['DISPLAY_PROPERTIES']['COM_BLACK'], $arItem['DISPLAY_PROPERTIES']['NEWPRODUCT'], $arItem['DISPLAY_PROPERTIES']['SALEPRODUCT']);

                        $old_price = 0;

                        if(     isset($arItem["PROPERTIES"]["OLD_PRICE"])
                            &&  isset($arItem["PROPERTIES"]["OLD_PRICE"]["~VALUE"])
                            &&  isset($arItem["PROPERTIES"]["OLD_PRICE"]["~VALUE"])
                            &&  isset($arItem["PROPERTIES"]["OLD_PRICE"]["~VALUE"])
                        ){

                            $old_price = trim($arItem["PROPERTIES"]["OLD_PRICE"]["~VALUE"]);
                            $old_price = str_ireplace(',','.',trim($old_price));
                            $old_price = (float)$old_price;

                        };

                        unset($arItem["PROPERTIES"]["OLD_PRICE"],$arItem["DISPLAY_PROPERTIES"]["OLD_PRICE"]);
                            ...
                            <div class="bx_catalog_item">
                                ...
                                ?>
                                    ...
                                    <div class="bx_catalog_item_images">
                                        ...
                                        if($old_price > 0
                                            && isset($minPrice['DISCOUNT_VALUE'])
                                            && !empty($minPrice['DISCOUNT_VALUE'])){

                                            $discount_val =  str_ireplace(',','.',trim($minPrice['DISCOUNT_VALUE']));
                                            $discount_val = (float)$discount_val;

                                            if($old_price > $discount_val){

                                                $old_price_prct = 100 - round(( $discount_val / $old_price * 100),0);
                                        ...
                                        ?>
                                        <?php if(!empty($old_price_prct)): ?>
                                            <div class="item_old_price_bage">
                                                -<? echo($old_price_prct); ?>%
                                            </div>
                                        <?php endif; ?>
                                        <? if($newproduct){?>
                                    ...
                                    <div class="product_quantity_holder product_quantity_holder<?=$arItem['ID'];?>">
                                        ...
                                        if($old_price > 0
                                            ...
                                            $old_price_val =  CCurrencyLang::CurrencyFormat($old_price,$minPrice['CURRENCY']);
                                        }

                                        ?>
                                        <?php if(!empty($old_price_val)): ?>
        ...
        <?      break; ?>
        <?  case 'LIST': ?>
            <div class="linear clearfix" id="linear">
                <?
                ...
                foreach ($arResult['ITEMS'] as $key => $arItem){
                    ...
                    $strMainID = $this->GetEditAreaId($arItem['ID']);
                    ?>
                    <div class="row" id="<? echo $strMainID; ?>l">
                        <?
                        $this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], $strElementEdit);
                        $this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], $strElementDelete, $arElementDeleteParams);
                        ...
                        $arItemIDs = array(
                            'ID' => $strMainID.'l',
                            'PICT' => $strMainID.'_pictl',
                            'SECOND_PICT' => $strMainID.'_pictl',
                            'STICKER_ID' => $strMainID.'_stickerl',
                            'SECOND_STICKER_ID' => $strMainID.'_secondstickerl',
                            'QUANTITY' => $strMainID.'_quantityl',
                            'QUANTITY_DOWN' => $strMainID.'_quant_downl',
                            'QUANTITY_UP' => $strMainID.'_quant_upl',
                            'QUANTITY_MEASURE' => $strMainID.'_quant_measurel',
                            'BUY_LINK' => $strMainID.'_buy_linkl',
                            'BUY_LINK_ICON' => $strMainID.'_buy_link_iconl',
                            'BASKET_ACTIONS' => $strMainID.'_basket_actionsl',
                            'NOT_AVAILABLE_MESS' => $strMainID.'_not_availl',
                            'SUBSCRIBE_LINK' => $strMainID.'_subscribel',
                            'COMPARE_LINK' => $strMainID.'_compare_linkl',

                            'PRICE' => $strMainID.'_pricel',
                            'DSC_PERC' => $strMainID.'_dsc_percl',
                            'SECOND_DSC_PERC' => $strMainID.'_second_dsc_percl',
                            'PROP_DIV' => $strMainID.'_sku_treel',
                            'PROP' => $strMainID.'_prop_l',
                            'DISPLAY_PROP_DIV' => $strMainID.'_sku_propl',
                            'BASKET_PROP_DIV' => $strMainID.'_basket_propl',
                        );
                        ...
                        <div class="bx_catalog_item_images col-xs-12 col-sm-12 col-md-3 col-lg-3">
                            <?
                    ...
                    <?=GetMessage("CRL_SALE"); ?>
                        ...
                        <div class="col-xs-12 col-sm-7 col-md-6 col-lg-6 item-description">
                            <?
                            ...
                            if(
                                (isset($arItem["DETAIL_TEXT"]) && !empty($arItem["DETAIL_TEXT"]))
                                ||
                                (isset($arItem["PREVIEW_TEXT"]) && !empty($arItem["PREVIEW_TEXT"]))
                            ){
                                ...
                                if(empty($detail_text)){
                                    ...
                                    $arItem["DETAIL_TEXT"] = trim($arItem["DETAIL_TEXT"]);
                                    $detail_text = $arItem["DETAIL_TEXT"];
                                };

                                if($preview_length && (mb_strlen($detail_text) > $preview_length)){
                                    $detail_text = mb_substr($detail_text,0,$preview_length);
                                    if(($pos = mb_strrpos($detail_text," ")) !== false){
                                        $detail_text = mb_substr($detail_text,0,$pos);
                                    };
                                };
                            };

                            if(!empty($detail_text)){?>
                                <div class="bx_catalog_item_preview_text">
                                    <a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>">
                                        <? echo $detail_text; ?>&hellip;
                                    </a>
                                </div>
                            <?}?>
                            <?php if($quantity != ""){ ?>
                            ...
                            if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
                            {
                                ...
                                <div class="bx_catalog_item_controls row">
                                    <?
                                    ?>
                                    <div class="bx_catalog_item_buy_block bx_catalog_item_buy_block<?=$arItem['ID'];?><? if(!$arItem['CAN_BUY']): ?> hidden<? endif;?>">
                                        <?
                                        if ('Y' == $arParams['USE_PRODUCT_QUANTITY'])
                                        {
                                            ?>
                                        ...
                                        if(false){
                                            ?>
                                            <div class="col-lg-4 col-md-12 col-sm-12 col-xs-12 form-button-area hidden">
                                                <div class="form-inline">
                                                    <div class="form-group">
                                                        <div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
                                                            <a id="<? echo $arItemIDs['BUY_LINK']; ?>"<? if(isset($arItem['BUY_ID']) && !empty($arItem['BUY_ID']) && $arItem['BUY_ID'] != $arItem['ID']): ?> data-buy-id="<?=$arItem['BUY_ID'];?>"<? endif; ?> class="btn pull-right btn-cart<?=$arItem['ID'];?> btn-info" href="javascript:void(0)" rel="nofollow">
                                                                <?
                                                                if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY')
                                                                {
                                                                    echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
                            ...
                            ?>
                                ...
                                ?><script type="text/javascript">
                                //<!--
                                var <? echo $strObName; ?> = new JCCatalogSectionL(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
                                //-->
                            </script><?
                            }
                            else
                            ...
                            ?>          </div>
                        <div class="col-xs-12 col-sm-5 col-md-3 col-lg-3 item-links">
                            <?
                            $notavailable = !empty($minPrice['VALUE']) ? false : true;
                            $inStock = $notavailable || !$arItem['CAN_BUY'] ? false : true;

                            ?>
                            <div class="in-stock-label pull-right" id="in-stock-<?=$arItem["ID"];?>">
                            ...
                            <div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
                                <a id="<? echo $arItemIDs['BUY_LINK']; ?>"<? if(isset($arItem['BUY_ID']) && !empty($arItem['BUY_ID']) && $arItem['BUY_ID'] != $arItem['ID']): ?> data-buy-id="<?=$arItem['BUY_ID'];?>"<? endif; ?> class="btn pull-right btn-cart<?=$arItem['ID'];?> btn-info pull-right" href="javascript:void(0)" rel="nofollow">
                                    <?
                                    if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY'){
                                        echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
                                    } else {
                            ...
                            <noindex>
                                <div class="preorder<?=$arItem['ID'];?><?=($notavailable)?' display-none':'';?><?=(!$arItem['CAN_BUY'])?'':' hidden';?>">
                                    <button class="btn btn-danger pull-right"  data-toggle="modal" data-target="#modalOCBuy<?=$arItem['ID'];?>l" id="order-one-click<?=$arItem['ID'];?>l">
        			                    <span class="catalog_one_click_pre_order">
                                                <?php echo GetMessage('CATALOG_ONE_CLICK_PRE_ORDER'); ?>
                                        </span>
                                    </button>
                                    <div class="modal bs-example-modal-sx preorder-section-modal" id="modalOCBuy<?=$arItem['ID'];?>l" tabindex="-1" role="dialog" aria-labelledby="modalOCBuyLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-sm">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                ...
                                                <div class="modal-body">
                                                    <div class="order_one_click_form clearfix" id="order_one_click_form<?=$arItem['ID'];?>l">
                                                        <div id="oc_error<?=$arItem['ID'];?>l" class="clearfix hidden">
                                                            <div class="alert alert-danger text-left" role="alert">
        									                    <span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true">
        									                    </span>
                                                                <span class="sr-only">
                                                        ...
                                                        <div id="oc_order<?=$arItem['ID'];?>l" class="clearfix hidden alert alert-success" role="alert">
                                                        </div>
                                                        <div class="form-group input-group name-group clearfix">
        								                        <span class="input-group-addon">
        									                        <span class="glyphicon glyphicon-user" aria-hidden="true">
                                                            ...
                                                            <input type="text" id="PAYER_NAME<?=$arItem['ID'];?>l" name="PAYER_NAME" value="" placeholder="<?php echo GetMessage('OC_PAYER_NAME'); ?>" class="form-control" />
                                                            <span class="input-group-addon">
        									                        *
        									                    </span>
                                                        </div>
                                                        <div class="form-group input-group phone-group clearfix">
                                                            ...
                                                            <input type="text" id="PAYER_PHONE<?=$arItem['ID'];?>l" name="PAYER_PHONE" value="" placeholder="<?php echo GetMessage('OC_PAYER_PHONE'); ?>" class="form-control" />
                                                            <span class="input-group-addon">
                            									    *
                            									</span>
                                                        </div>
                                                        <div class="form-group input-group email-group clearfix">
                                                            ...
                                                            <input type="text" id="PAYER_EMAIL<?=$arItem['ID'];?>l" name="PAYER_EMAIL" value="" placeholder="<?php echo GetMessage('OC_PAYER_EMAIL'); ?>" class="form-control" />
                                                        </div>
                                                        <div class="form-group clearfix text-center">
                                                            <button id="PAY_ONE_CLICK<?=$arItem['ID'];?>l" class="btn btn-info">
        									                        <span class="catalog_one_click_pre_order">
                                                                        <?php echo GetMessage('CATALOG_ONE_CLICK_PRE_ORDER'); ?>
                                                                    </span>
                                                            </button>
                                                        ...
                                                        <input type="hidden" id="PRODUCT_ID<?=$arItem['ID'];?>l" name="PRODUCT_ID" value="<?php echo $arItem['ID']; ?>" />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <script type="text/javascript">
                                ...
                                $(function(){
                                    ...
                                    $('#PAYER_PHONE<?=$arItem['ID'];?>l').attr('autocomplete',false);
                                    $('#PAYER_PHONE<?=$arItem['ID'];?>l').attr('placeholder', phonePlaceholder);
                                    ...
                                    $('#PAYER_PHONE<?=$arItem['ID'];?>l').mask(phoneMask,maskOptions);

                                    $("#PAY_ONE_CLICK<?=$arItem['ID'];?>l").bind("click",function(event){

                                        var ajax_cart_action = '/ajax_cart/preorder.php';

                                        event.preventDefault();

                                        $('#order_one_click_form<?=$arItem['ID'];?>l .form-group').removeClass("has-error");
                                        $('#order_one_click_form<?=$arItem['ID'];?>l .form-group').removeClass("has-success");

                                        var ocProductId = $("#PRODUCT_ID<?=$arItem['ID'];?>l")[0].value;

                                        var ocPayerName = $("#PAYER_NAME<?=$arItem['ID'];?>l")[0].value;
                                        ocPayerName	= $.trim(ocPayerName);

                                        var hasErrors = false;

                                        if(ocPayerName.length > 3){
                                            $('#modalOCBuy<?=$arItem['ID'];?>l .name-group').addClass('has-success');
                                        } else {
                                            $('#modalOCBuy<?=$arItem['ID'];?>l .name-group').addClass('has-error');
                                            hasErrors = true;
                                        };

                                        var ocPayerPhone = $("#PAYER_PHONE<?=$arItem['ID'];?>l")[0].value;
                                        ocPayerPhone = $.trim(ocPayerPhone);


                                        if(ocPayerPhone.length  > 3){
                                            $('#modalOCBuy<?=$arItem['ID'];?>l .phone-group').addClass('has-success');
                                        } else {
                                            $('#modalOCBuy<?=$arItem['ID'];?>l .phone-group').addClass('has-error');
                                            hasErrors = true;
                                        };

                                        var ocPayerEmail = $("#PAYER_EMAIL<?=$arItem['ID'];?>l")[0].value;
                                        ocPayerEmail = $.trim(ocPayerEmail);
                                        ...
                                        if((ocPayerEmail.length > 5
                                            ...
                                            $('#modalOCBuy<?=$arItem['ID'];?>l .email-group').addClass('has-success');
                                        } else {
                                            $('#modalOCBuy<?=$arItem['ID'];?>l .email-group').addClass('has-error');
                                            hasErrors = true;
                                        };

                                        $('#oc_error<?=$arItem['ID'];?>l .errors')[0].innerHTML = "";
                                        $('#oc_error<?=$arItem['ID'];?>l,#oc_order<?=$arItem['ID'];?>l').addClass("hidden");

                                        BX.closeWait();

                                        if(!hasErrors){

                                            $("#PAY_ONE_CLICK<?=$arItem['ID'];?>l").attr('disabled',true);
                                            BX.showWait();
                                            ...
                                            BX.ajax({
                                                ...
                                                onsuccess: function(result){
                                                    ...
                                                    if(result.ERROR){
                                                        $('#oc_error<?=$arItem['ID'];?>l').removeClass("hidden");
                                                        $("#PAY_ONE_CLICK<?=$arItem['ID'];?>l").attr('disabled',false);
                                                        ...
                                                        $('#modalOCBuy<?=$arItem['ID'];?>l .form-group').css('display','none');

                                                        $('#oc_order<?=$arItem['ID'];?>l')[0].innerHTML = resultAnswer.replace('#',result.ORDER_ID);
                                                        $('#oc_order<?=$arItem['ID'];?>l').removeClass("hidden");
                                                    };
                                                },
                                                onfailure: function(type, e){
                                                    BX.closeWait();
        ...
        <div class="pagination row clearfix clear" id="section-bottom-pagination">
            <div class="col-xs-3 col-sm-3 col-md-3 col-lg-4">
                <span class="hidden-xs hidden-sm hidden-md"><?php echo GetMessage("SHOW_COUNT"); ?></span>
                <select name="PAGE_ELEMENT_COUNT" id="PAGE_ELEMENT_COUNT" class="select-field selectpicker">
                    <?php foreach($pager as $page): ?>
                        <option value="<?php echo $page; ?>"<?php if($page == $element_count): ?> selected="selected"<?php endif; ?>>
                            <?php echo $page; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <span class="hidden-xs hidden-sm hidden-md"><?php echo GetMessage("PRODUCT_BY_PAGE"); ?></span>
            </div>
            <div class="col-xs-9 col-sm-9 col-md-9 col-lg-8 pull-right">

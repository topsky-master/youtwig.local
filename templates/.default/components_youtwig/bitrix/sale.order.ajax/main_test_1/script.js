...
function remapPhoneMask(){
    ...
    if($('#ORDER_PROP_3').get(0)){

        var idDefaultRussia = true;
        var isCitySelected = $('.loc.city').get(0);
        if(isCitySelected && isCitySelected.value){

            isCitySelectedTitle = $.trim(isCitySelected.value);

            if(isCitySelectedTitle
                && isCitySelectedTitle.indexOf(',') !== -1){

                var countrySelected = isCitySelectedTitle.split(',');


                if(countrySelected && countrySelected.length){

                    countrySelected = countrySelected[countrySelected.length-1];

                    countrySelected = $.trim(countrySelected);


                };

            } else if(isCitySelectedTitle != ""){

                countrySelected = isCitySelectedTitle;
            ...
            if($.inArray( countrySelected, orderCountries) !== -1){
                ...
                try{
                    ...
                    $('#ORDER_PROP_3').cleanVal();
                    $('#ORDER_PROP_3').unmask();
                    $('#ORDER_PROP_3').unbind('keydown');

                    $('#ORDER_PROP_3').tooltip('destroy');
                    $('#ORDER_PROP_3').removeClass('has_error');
                    $('#ORDER_PROP_3').removeClass('has_order_error');
                ...
                $('#ORDER_PROP_3').attr('autocomplete',false);

                $('#ORDER_PROP_3').attr('placeholder', phonePlaceholder);

                switch(countrySelected){
                    case 'Россия':

                        var spValue = $('#ORDER_PROP_3').val();
                        var iMaxLength = 12;
                        iMaxLength = spValue[0] == '+' ? 12 : 11;

                        if($('#ORDER_PROP_3').val().length < iMaxLength){

                            $('.ORDER_PROP_3').find('.has-success').removeClass('has-success').addClass('has_order_error');
                            $('.ORDER_PROP_3').find('.has_tooltip').addClass('has_error');
                        ...
                        if($('#ORDER_PROP_3').val().length > (iMaxLength - 1)){

                            $('.ORDER_PROP_3').find('.has_order_error').removeClass('has_order_error').addClass('has-success');
                            $('.ORDER_PROP_3').find('.has_tooltip').removeClass('has_error');
                        ...
                        var maskOptions =  {
                            ...
                            onKeyPress: function(cep, event, currentField, options){
                                ...
                                if($(currentField).get(0)
                                    ...
                                    && $(currentField).attr('id') == 'ORDER_PROP_3'
                                    ...
                                    $('#checkPhoneModal button').on('click',function(event){
                                        ...
                                        $('#ORDER_PROP_3').val($('#checkphone').val());
                        ...
                        $('#ORDER_PROP_3').mask(phoneMask, maskOptions);
                        break;
                    default:
...
function destroyLocPopover() {
    if (locPopoverShow) {
        try {
            $('#ORDER_FORM input.loc.city').popover('destroy');
            $('.popover.in').remove();
        } catch (e) {
...
function createLocPopover() {
    ...
    var popoverContent = '<div class="message"><p>Это ваш город?</p><div class="buttons text-center"><button class="yes btn-info btn" onclick="$.cookie(\'locationSelected\',true);$(\'#ORDER_FORM input.loc.city\').popover(\'hide\');$(\'#ORDER_CONFIRM_BUTTON\').prop(\'disabled\',false);">Да</button><button class="no btn btn-danger" onclick="$.cookie(\'locationSelected\',null);$(\'#ORDER_FORM input.loc.city\').popover(\'hide\');$(\'#ORDER_CONFIRM_BUTTON\').prop(\'disabled\',\'true\');">Нет</button></div></div></div>';

    if (!locPopoverShow
        && $.trim($('#ORDER_FORM input.loc.city').val()) != "") {

        $('#ORDER_FORM input.loc.city').popover({
        ...
        $('#ORDER_FORM input.loc.city').popover('show');
...
function afterFormReload() {
    ...
    try{
        locPopoverShow = true;
        $('#ORDER_FORM input.loc.city').blur();
        $('#ORDER_FORM input.loc.city').popover('destroy');
        $('.popover.in').remove();
    ...
    locInterval = setInterval(function(){
        ...
        if($('#ORDER_FORM input.loc.city').get(0)){
            ...
            if(locationSelectedConfirmed != 'true') {
                ...
                $('#ORDER_FORM input.loc.city').on("focus",destroyLocPopover);

                $('#ORDER_FORM input.loc.city').on("blur", createLocPopover);

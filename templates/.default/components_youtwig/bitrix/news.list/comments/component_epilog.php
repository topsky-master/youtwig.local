...
if(isset($arResult['AMP_HTML_URL'])){

    $amp_html_url = $arResult['AMP_HTML_URL'];

    if(isset($amp_html_url) && !empty($amp_html_url)){

        $canonical_path = $amp_html_url;

        $amp_html_url = (preg_match('~http(s*?)://~',$amp_html_url) == 0 ? (IMPEL_PROTOCOL.IMPEL_SERVER_NAME.$amp_html_url) : $amp_html_url);

		//$APPLICATION->AddHeadString('<link rel="amphtml" href="'.$amp_html_url.'" />',true);

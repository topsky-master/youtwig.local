...
if($arResult["bDescPageNumbering"] === true)
{
	...
	?>
		...
		<?else:?>
			<a href="<?=$sPrevHref;?>" id="<?=$ClientID?>_previous_page">
				<?=GetMessage("nav_prev")?>
			</a>
		<?endif;?>
	<?
	...
	?>...<?
		...
		$link_next						= $arResult["sUrlPath"].'?'.$strNavQueryString.('PAGEN_'.$arResult["NavNum"].'='.$arResult["nStartPage"]);
		$link_next						= (preg_match('~http(s*?)://~',$link_next) == 0) ? IMPEL_PROTOCOL.IMPEL_SERVER_NAME.$link_next : $link_next;

		$link_prev						= $arResult["sUrlPath"].((!empty($strNavQueryString) && !$arResult["NavNum"] > 1) ? '?'.$strNavQueryString.($arResult["NavNum"] > 1 ? ('PAGEN_'.$arResult["NavNum"].'='.$arResult["nStartPage"]) : '') : '');
		$link_prev						= (preg_match('~http(s*?)://~',$link_prev) == 0) ? IMPEL_PROTOCOL.IMPEL_SERVER_NAME.$link_prev : $link_prev;

		if ($arResult["nStartPage"] + 1 == $arResult["NavPageNomer"]) {
			$APPLICATION->AddHeadString('<link rel="next" href="'.$link_next.'" />');
		} elseif ($arResult["nStartPage"] - 1 == $arResult["NavPageNomer"]) {
			$APPLICATION->AddHeadString('<link rel="prev" href="'.$link_prev.'" />');
		}
	...
	<?else:?>
		<a href="<?=$sNextHref;?>" id="<?=$ClientID?>_next_page">
			<?=GetMessage("nav_next")?>
		</a>
	<?endif;?>
	<?php 

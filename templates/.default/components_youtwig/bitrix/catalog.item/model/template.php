...
if (isset($arResult['ITEM']))
{
    ...
    if($rProp && $aProp = $rProp->GetNext()){
        $bIsModelPLink = $aProp['VALUE_XML_ID'] == 'Y' ? true : false;
    }

    if ($bIsModelPLink) {
        $item['DETAIL_PAGE_URL'] = preg_replace('~(/model/[^/]+?/)(.*)~isu',"$1",$APPLICATION->GetCurDir()).$item['ID'].'/';
    }
	...
	$manufacturer = isset($arParams['MANUFACTURER'])
            ...
            && isset($item['DISPLAY_PROPERTIES']['MANUFACTURER']['VALUE'])
            && !empty($item['DISPLAY_PROPERTIES']['MANUFACTURER']['VALUE'])
				? trim($item['DISPLAY_PROPERTIES']['MANUFACTURER']['VALUE'])
                : '');

            if(empty($manufacturer)){

                if(file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/tmp/filters_mt_cache.php')
                    && !(isset($amLines) && !empty($amLines))
                ) require $_SERVER['DOCUMENT_ROOT'].'/bitrix/tmp/filters_mt_cache.php';

                if(!isset($amLines[$item['ID']])){
    ...
    if(!$arParams['IS_VERSION']){

        if(isset($arParams['MODEL_NAME'])
            && !empty($arParams['MODEL_NAME'])
            && $arParams['MODEL_NAME'] != "-"){

            $productTitle = trim($arParams['MODEL_NAME']);

        } else {

            $typeproduct = isset($item['DISPLAY_PROPERTIES'])
            && isset($item['DISPLAY_PROPERTIES']['TYPEPRODUCT'])
            && isset($item['DISPLAY_PROPERTIES']['TYPEPRODUCT']['VALUE'])
            && !empty($item['DISPLAY_PROPERTIES']['TYPEPRODUCT']['VALUE'])
                ? trim($item['DISPLAY_PROPERTIES']['TYPEPRODUCT']['VALUE'])
                : '';

            if(empty($typeproduct)){

                if(file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/tmp/filters_mt_cache.php')
                    && !(isset($atLines) && !empty($atLines))
                ) require $_SERVER['DOCUMENT_ROOT'].'/bitrix/tmp/filters_mt_cache.php';

                if(!isset($atLines[$item['ID']])){
                    ...
                    if($rProp && $aProp = $rProp->GetNext()){
                        ...
                        $typeproduct = $aProp['VALUE_ENUM'];
                    }

                } else {
                    $typeproduct = $atLines[$item['ID']];
                }
            }
            ...
            $sIndCode = isset($arParams['INDCODE'])
            && !empty($arParams['INDCODE'])
                ? trim($arParams['INDCODE'])
                : '';
            ...
            if(!empty($typeproduct)
                ...
                $productTitle = $typeproduct;
                $productTitle.= (!empty($productTitle) ? ' '.GetMessage('CATALOG_FOR').' ' : '') . $manufacturer;
                $productTitle.= (!empty($productTitle) ? ' ' : ' '.GetMessage('CATALOG_FOR').' ') . $model_new;
                $productTitle.= (!empty($sIndCode) ? ' '.$sIndCode : ' ');
        ...
        $typeproduct = isset($item['DISPLAY_PROPERTIES'])
        && isset($item['DISPLAY_PROPERTIES']['TYPE_OF_PRODUCT'])
        && isset($item['DISPLAY_PROPERTIES']['TYPE_OF_PRODUCT']['VALUE'])
        && !empty($item['DISPLAY_PROPERTIES']['TYPE_OF_PRODUCT']['VALUE'])
            ? trim($item['DISPLAY_PROPERTIES']['TYPE_OF_PRODUCT']['VALUE'])
            : '';
        ...
        $model_new = (isset($item['DISPLAY_PROPERTIES'])
        && isset($item['DISPLAY_PROPERTIES']['MODEL_NEW_LINK'])
        && isset($item['DISPLAY_PROPERTIES']['MODEL_NEW_LINK']['VALUE'])
        && !empty($item['DISPLAY_PROPERTIES']['MODEL_NEW_LINK']['VALUE'])
            ? trim($item['DISPLAY_PROPERTIES']['MODEL_NEW_LINK']['VALUE'])
            : '');
        ...
        if(empty($model_new)){
            ...
            if($rProp && $aProp = $rProp->GetNext()){
                ...
                $lDb = CIBlockElement::GetByID($aProp['VALUE']);

                if($lDb && $aDb = $lDb->GetNext()){
                    $model_new = $aDb['NAME'];
                }
        ...
        if(!empty($typeproduct)
            ...
            $productTitle = $typeproduct;
            $productTitle.= (!empty($productTitle) ? ' ' : '') . $manufacturer;
            $productTitle.= (!empty($productTitle) ? ' ' : '') . $model_new;
	...
	?>
    <div class="product-item" id="<?=$areaId?>" data-entity="item" itemscope="" itemtype="http://schema.org/Product">
        <?
        ...
        $jsParams = array(
            ...
            'BASKET' => array(
                'ADD_PROPS' => $arParams['ADD_DISPLAY_PROPERTIES_TO_BASKET'] === 'Y',
                'QUANTITY' => $arParams['PRODUCT_QUANTITY_VARIABLE'],
                'PROPS' => $arParams['PRODUCT_PROPS_VARIABLE'],
                'EMPTY_PROPS' => empty($item['PRODUCT_DISPLAY_PROPERTIES']),

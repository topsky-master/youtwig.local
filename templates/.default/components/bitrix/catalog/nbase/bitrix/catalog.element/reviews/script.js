function bindElementSlider(){
    ...
    if($("#pslider").get(0)){
        ...
        if($('#psslider').get(0)){
            ...
            $('#pslider').on('slide.bs.carousel', function (item) {
                var slideTo = parseInt($(item.relatedTarget).attr('data-slide-to'));
                if(!isNaN(slideTo)){
                    $('#psslider').carousel(slideTo);
                }
            });
            ...
            if($('.psthumbs').get(0)) {
                $('#pslider').on('slide.bs.carousel', function (item) {
                    var slideTo = parseInt($(item.relatedTarget).attr('data-slide-to'));
                    if (!isNaN(slideTo)) {
                        $('.psthumbs').find('.item').removeClass('active').eq(slideTo).addClass('active');
                    }
                });
            }
        ...
        if($('.psthumbs').get(0)){
            $('.psthumbs img').click(function(){
                var slideTo = parseInt($(this).attr('data-slide-to'));
                if(!isNaN(slideTo)){
                    $('#pslider').carousel(slideTo);
                }
            });
        }
        ...
        $(this).ekkoLightbox(
            {
                ...
                onNavigate: function(direction, itemIndex){
                    ...
                    if($('#pslider').get(0)){
                        $('#pslider').carousel(itemIndex);
                    }

                    if($('#psslider').get(0)){
                        $('#psslider').carousel(itemIndex);
                    }
                }
            }
        );
...
function bindQuickPhoneForm(){
    ...
    if(!isNaN(ocProductId)){
        ...
        $('#oqCallbackme').bind('click',function(){
            ...
            if($('#oqQuickPhone').val() == ''){
                ...
                jQuery.ajax({
                    ...
                    if(result.ERROR){
                        ...
                        var resultAnswer = $.trim($('#oqResultOk').text());

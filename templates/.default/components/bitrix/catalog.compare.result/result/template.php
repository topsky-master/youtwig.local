<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?>
		...
		<a name="compare_table"></a>
			...
			<form action="<?=$APPLICATION->GetCurPage()?>" method="get" class="compare_properties">
				<h6><?=GetMessage("CATALOG_REMOVE_FEATURES")?>:</h6>
				<?foreach($arResult["SHOW_PROPERTIES"] as $arProperty):?>
					<label><input type="checkbox" name="pr_code[]" value="<?=$arProperty["CODE"]?>" /><?=$arProperty["NAME"]?></label>
				<?endforeach?>
				<?foreach($arResult["SHOW_OFFER_FIELDS"] as $code):?>
					<label><input type="checkbox" name="of_code[]" value="<?=$code?>" /><?=GetMessage("IBLOCK_FIELD_".$code)?></label>
				<?endforeach?>
				<?foreach($arResult["SHOW_OFFER_PROPERTIES"] as $arProperty):?>
					<label><input type="checkbox" name="op_code[]" value="<?=$arProperty["CODE"]?>" /><?=$arProperty["NAME"]?></label>
				<?endforeach?>
				<input type="hidden" name="action" value="DELETE_FEATURE" />
				<input type="hidden" name="IBLOCK_ID" value="<?=$arParams["IBLOCK_ID"]?>" />
				<div>
					<input type="submit" value="<?=GetMessage("CATALOG_REMOVE_FEATURES")?>" class="btn btn-info">
				</div>
				</form>
			<?endif?>
			...
			<div>
			<input type="submit" value="<?=GetMessage("CATALOG_REMOVE_PRODUCTS")?>" class="btn btn-info" />
			</div>
		...
		<form action="<?=$APPLICATION->GetCurPage()?>" method="get">
			...
			<select name="id" class="select-field selectpicker">
			<?foreach($arResult["ITEMS_TO_ADD"] as $ID=>$NAME):?>
				<option value="<?=$ID?>"><?=$NAME?></option>
			<?endforeach?>
			</select>
			<input type="submit" value="<?=GetMessage("CATALOG_ADD_TO_COMPARE_LIST")?>" class="btn btn-info" />
		</form>
		<?endif?>
</div>

...
if (mb_strlen($arResult["opened"]) > 0)
{
...
<div class="profile-link profile-user-div-link">
	<h3>
		<a title="<?=GetMessage("REG_SHOW_HIDE")?>" href="javascript:void(0)" onclick="SectionClick('reg')">
			<span>-</span><?=GetMessage("REG_SHOW_HIDE")?>
		</a>
	</h3>
</div>
<div class="profile-block-<?=mb_strpos($arResult["opened"], "reg") === false ? "hidden" : "shown"?>" id="user_div_reg">
<table class="profile-table data-table table table-striped table-bordered table-hover">
	<tbody>
	<?
	if($arResult["ID"]>0)
	{
	?>
		<?
		if (mb_strlen($arResult["arUser"]["TIMESTAMP_X"])>0)
		{
		?>
		<tr>
		...
		?>
		<?
		if (mb_strlen($arResult["arUser"]["LAST_LOGIN"])>0)
		{
		?>
		<tr>
...
<?if($arResult["TIME_ZONE_ENABLED"] == true):?>
	...
	<tr>
		...
		<td>
			<select name="AUTO_TIME_ZONE" onchange="this.form.TIME_ZONE.disabled=(this.value != 'N')" class="select-field selectpicker">
	...
	<tr>
		...
		<td>
			<select name="TIME_ZONE"<?if($arResult["arUser"]["AUTO_TIME_ZONE"] <> "N") echo ' disabled="disabled"'?> class="select-field selectpicker">
...
<div class="profile-link profile-user-div-link">
	<h3>
		<a title="<?=GetMessage("USER_SHOW_HIDE")?>" href="javascript:void(0)" onclick="SectionClick('personal')">
			<span>+</span><?=GetMessage("USER_PERSONAL_INFO")?>
		</a>
	</h3>	
</div>
<div id="user_div_personal" class="profile-block-<?=mb_strpos($arResult["opened"], "personal") === false ? "hidden" : "shown"?>">
<table class="data-table profile-table table table-striped table-bordered table-hover">
	<tbody>
		<tr>
		...
		<tr>
			...
			<td><select name="PERSONAL_GENDER" class="select-field selectpicker">
		...
		<tr>
			...
			<?
			if (mb_strlen($arResult["arUser"]["PERSONAL_PHOTO"])>0)
			{
			?>
			<br />
		...
		<tr>
			...
			<td><?php echo preg_replace('~<select ~i','<select class="select-field selectpicker" ',$arResult["COUNTRY_SELECT"]); ?></td>
		</tr>
		<tr>
...
<div class="profile-link profile-user-div-link">
	<h3>
		<a title="<?=GetMessage("USER_SHOW_HIDE")?>" href="javascript:void(0)" onclick="SectionClick('work')">
			<span>+</span><?=GetMessage("USER_WORK_INFO")?>
		</a>
	</h3>
</div>
...
<table class="data-table profile-table table table-striped table-bordered table-hover">
	<tbody>
		...
		<tr>
			...
			<?
			if (mb_strlen($arResult["arUser"]["WORK_LOGO"])>0)
			{
			?>
		...
		<tr>
			<td><?=GetMessage('USER_COUNTRY')?></td>
			<td><?php echo preg_replace('~<select ~i','<select class="select-field selectpicker" ',$arResult["COUNTRY_SELECT_WORK"]);?></td>
		</tr>
		<tr>
			<td><?=GetMessage('USER_STATE')?></td>
...
<div class="profile-link profile-user-div-link">
	<h3>	
		<a title="<?=GetMessage("USER_SHOW_HIDE")?>" href="javascript:void(0)" onclick="SectionClick('forum')">
			<span>+</span><?=GetMessage("forum_INFO")?>
		</a>
	</h3>
</div>
...
<table class="data-table profile-table table table-striped table-bordered table-hover">
	<tbody>
		...
		<tr>
			...
			<?
			if (mb_strlen($arResult["arForumUser"]["AVATAR"])>0)
			{
			?>
...
<div class="profile-link profile-user-div-link">
	<h3>	
		<a title="<?=GetMessage("USER_SHOW_HIDE")?>" href="javascript:void(0)" onclick="SectionClick('blog')">
			<span>+</span><?=GetMessage("blog_INFO")?>
		</a>
	</h3>
</div>
...
<table class="data-table profile-table table table-striped table-bordered table-hover">
	<tbody>
		...
		<tr>
			...
			<?
			if (mb_strlen($arResult["arBlogUser"]["AVATAR"])>0)
			{
			?>
	...
	<div class="profile-link profile-user-div-link">
		<h3>	
			<a title="<?=GetMessage("USER_SHOW_HIDE")?>" href="javascript:void(0)" onclick="SectionClick('learning')">
				<span>+</span><?=GetMessage("learning_INFO")?>
			</a>
		</h3>
	</div>
	...
	<div class="profile-link profile-user-div-link">
		<h3>	
			<a title="<?=GetMessage("USER_SHOW_HIDE")?>" href="javascript:void(0)" onclick="SectionClick('admin')">
				<span>+</span><?=GetMessage("USER_ADMIN_NOTES")?>
			</a>
		</h3>
	</div>
	...
	<div class="profile-link profile-user-div-link">
		<h3>	
			<a title="<?=GetMessage("USER_SHOW_HIDE")?>" href="javascript:void(0)" onclick="SectionClick('user_properties')">
				<span>+</span><?=mb_strlen(trim($arParams["USER_PROPERTY_NAME"])) > 0 ? $arParams["USER_PROPERTY_NAME"] : GetMessage("USER_TYPE_EDIT_TAB")?>
			</a>
		</h3>
	</div>
	...
	<p><input class="btn btn-primary btn-lg" type="submit" name="save" value="<?=(($arResult["ID"]>0) ? GetMessage("MAIN_SAVE") : GetMessage("MAIN_ADD"))?>">&nbsp;&nbsp;<input class="btn btn-danger btn-lg" type="reset" value="<?=GetMessage('MAIN_RESET');?>"></p>
</form>
<?

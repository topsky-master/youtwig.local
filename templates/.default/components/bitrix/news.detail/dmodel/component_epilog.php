...
//echo '<p class="alert alert-info">Tecт нового шаблона из-под админа</p>';
...
if(isset($arResult["DISPLAY_PROPERTIES"]["SIMPLEREPLACE_VIEW"]["DISPLAY_VALUE"])
    ...
    if(empty($arResult["DISPLAY_PROPERTIES"]["SIMPLEREPLACE_VIEW"]["DISPLAY_VALUE"]) && !empty($curView)) {
        unset($products);
        global $USER;
        $filterError = $hasFilter = true;
    }
    ...
    if(isset($matches[1]) && !empty($matches[1])){
        ...
        foreach($arResult["DISPLAY_PROPERTIES"]["SIMPLEREPLACE_INDCODE"]["DISPLAY_VALUE"]
            ...
            if($productCode != $curIndcode){
                unset($arResult["DISPLAY_PROPERTIES"]["SIMPLEREPLACE_VIEW"]["VALUE"][$productNum]);
                unset($products[$productNum]);
            } else {
        ...
        if(empty($arResult["DISPLAY_PROPERTIES"]["SIMPLEREPLACE_INDCODE"]["DISPLAY_VALUE"]) && !empty($curIndcode)) {
            unset($products);
            $filterError = $hasFilter = true;
        }
...
$arResult['models_h1'] = str_ireplace(',',' ',$arResult['models_h1']);
$arResult['default_title'] = str_ireplace(',',' ',$arResult['default_title']);
...
if(preg_match('~/view/([^/]+?)/~isu',$curPage,$matches) &&  !$isVersion){
    ...
    if(isset($matches[1]) && !empty($matches[1])){
        ...
        <div class="view-preview row" id="view-preview">
            ...
            foreach($arResult["DISPLAY_PROPERTIES"]["SIMPLEREPLACE_VIEW"]["DISPLAY_VALUE"]
                ...
                if($curView == $productCode){
                    ...
                    $dbRes = CIBlockElement::GetByID($arResult["PROPERTIES"]["SIMPLEREPLACE_VIEW"]["VALUE"][$productNum]);
                    ...
                    if($dbRes){
                        ...
                        $spreview_picture = $preview_picture;
        ...
        <p id="hide-scheme" class="hide-scheme">
            <a href="<?=preg_replace('~/view/[^/]+?/~isu','/',$basePage);?>">
                                    <span>
...
<div class="<?php if($isVersion): ?>about-model-version <? endif; ?>about-model-area clearfix">
    ...
    viewModelComments($arResult["DISPLAY_PROPERTIES"]["type_of_product"]['VALUE'],$arResult["DISPLAY_PROPERTIES"]["manufacturer"]["VALUE"],$products);
        ...
        <?php if(!$isVersion): ?>
            <div class="col-md-3 col-sm-12 col-xs-12 about-model-info">
                <div class="about-model-download">
                    ...
                    <?php if(isset($arResult['INDCODE']) || (!empty($svFilter))) { ?>
                        <div class="indcode col-md-12 col-sm-6">
                            ...
                            <?php if(isset($arResult['INDCODE'])){ ?>
                                ...
                                <ul class="indcode-list">
                                    ...
                                    foreach($arResult['INDCODE'] as $value => $linkPage) {

                                        if(preg_match('~(view/[^/]+/)+?(indcode/[^/]+?/)~isu',$linkPage)){
                                            $linkPage = preg_replace('~(view/[^/]+/)+?(indcode/[^/]+?/)~isu',"$2$1",$linkPage);
                                        }
                                        ?>
                                        <li>
                                            <a href="<?=$linkPage;?>">
                ...
                <div class="views row">
                    ...
                    foreach($views as $viewNum => $viewId){
                        ...
                        if($dbRes){
                            ...
                            $code = trim(CUtil::translit(trim(strip_tags($arResult["DISPLAY_PROPERTIES"]["SIMPLEREPLACE_VIEW"]["DISPLAY_VALUE"][$viewNum])), LANGUAGE_ID, $trParams));
...
if(isset($arResult['CODE'])
    ...
    $sDir = '/amp/'.trim($sDir,'/').'/';
    //$APPLICATION->AddHeadString('<link rel="amphtml" href="'.IMPEL_PROTOCOL.IMPEL_SERVER_NAME.$sDir.'" />');

}

$baseOrgPage = $APPLICATION->GetCurDir();

if(preg_match('~/(view)/~isu',$baseOrgPage) === 1 || preg_match('~/(indcode)/~isu',$baseOrgPage) === 1){
    $APPLICATION->SetPageProperty("robots", "noindex, nofollow");
}


if(preg_match('~/typeproduct[^/]+?/~isu',$baseOrgPage) === 1){
...
if(preg_match('~(view/[^/]+/)+?(indcode/[^/]+?/)~isu',$baseOrgPage)){

    $baseOrgPage = preg_replace('~(view/[^/]+/)+?(indcode/[^/]+?/)~isu',"$2$1",$baseOrgPage);
    $baseOrgPage = str_ireplace(IMPEL_PROTOCOL.IMPEL_SERVER_NAME,'',$baseOrgPage);
    $baseOrgPage = trim($baseOrgPage,'/');
    $baseOrgPage = '/'.$baseOrgPage.'/';

    LocalRedirect($baseOrgPage);
...
if($filterError && $hasFilter){
    ...
    if ($currentURL != '/model/'.$arResult['CODE'].'/') {
        LocalRedirect('/model/'.$arResult['CODE'].'/');
    } else {
...
?><input type="hidden" id="bpage" value="<?=$basePage;?>" />
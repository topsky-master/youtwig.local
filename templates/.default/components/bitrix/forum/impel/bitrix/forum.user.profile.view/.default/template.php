...
<?
		if (mb_strlen($arResult["FORUM_USER"]["DESCRIPTION"]) > 0):
?>
...
if ($arResult["USER"]["SHOW_PANELS"]["user_prop"] != "N" && $arResult["USER_PROPERTIES"]["SHOW"] == "Y"):
	...
	foreach ($arResult["USER_PROPERTIES"]["DATA"] as $FIELD_NAME => $arUserField):
		...
		if($arUserField["ENTITY_VALUE_ID"] < 1 && mb_strlen($arUserField["SETTINGS"]["DEFAULT_VALUE"]) > 0)
...
<?
		...
		if ($arParams["SHOW_MAIL"] == "Y" && mb_strlen($arResult["USER"]["EMAIL"]) > 0):
			$bEmptyCell = false;
?>
...
<?
		...
		if ($arResult["SHOW_ICQ"] == "Y" && mb_strlen($arResult["USER"]["PERSONAL_ICQ"]) > 0):
			$bEmptyCell = false;
?>
...
?>
								&nbsp;&nbsp;<span class="forum-user-messages">
									<noindex><a rel="nofollow" href="<?=$arResult["arTopic"]["read"]?>" title="<?=htmlspecialcharsEx($arResult["arTopic"]["~TITLE"])?><?
										if (mb_strlen($arResult["arTopic"]["~DESCRIPTION"])>0):

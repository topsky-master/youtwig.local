...
?>
    <div class="item-images">
        <input type="hidden" class="product_cart_name" value="<?=$productTitle?>" />
        <?php if(isset($item['ARTNUMBER'])
            && !empty($item['ARTNUMBER'])): ?>
            <div class="item-scu">
                <meta itemprop="sku" content="<?=$item['ARTNUMBER']['VALUE'];?>">
                <meta itemprop="brand" content="<?=$item['MANUFACTURER'];?>">
                <?=$item['ARTNUMBER']['NAME'];?>
                <span>
        ...
        <?php endif; ?>
        <?php if(isset($arParams['POSITION'])
            && !empty($arParams['POSITION'])): ?>
            <div class="item-scu position">
                <?=GetMessage('TMPL_POSITION_NUMBER');?>
                <span>
            <?=$arParams['POSITION'];?>
        </span>
            </div>
        ...
        if(Bitrix\Main\Loader::includeModule('api.uncachedarea')) {
            CAPIUncachedArea::includeFile(
                ...
                array(
                    ...
                    'TYPE_OF_PRODUCT' => isset($arParams['TYPE_OF_PRODUCT']) ? $arParams['TYPE_OF_PRODUCT'] : null,
                    'MANUFACTURER' => isset($arParams['MANUFACTURER']) ? $arParams['MANUFACTURER'] : null,
                    'MODEL_NAME' => isset($arParams['MODEL_NEW']) ? $arParams['MODEL_NEW'] : null
                )
            );
        }

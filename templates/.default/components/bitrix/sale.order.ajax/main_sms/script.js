...
function afterFormReload() {
    ...
    if ($('#changeToLegal').get(0)) {
        ...
        setTimeout(function(){
            if (!$('body').hasClass('setted') && !sCheck) {
                $('body').addClass('setted');
                if ($('#ID_PROFILE_ID').get(0)) {
                    $('#ID_PROFILE_ID').trigger('change');
                }
            }
        },80);
    ...
    if($('#passcodeModal').get(0)
        ...
        $('.PHONE input[type="text"]').bind('blur',function(){
        ...
        $('#passcodeModal').on('hide.bs.modal', function () {

            $('div.PHONE .form-control').val('');
            $('div.PHONE').addClass('required has_order_error').removeClass('has-success');
            $('div.PHONE .form-control').addClass('has_error');

...
var orderCountries = ['Россия'];
var orderCountriesPhonePlaceholder = ['+79_________'];
var orderCountriesPhoneMask = ['+79000000000'];
var checkmodelHidden = false;
var checkErrorClass = 'has-error';
...
function remapCount() {
    ...
    if ($('.order_quantity').get(0)) {
        ...
        $('.order_quantity').on('change',function(){
            doRecount($(this).attr('data:id'),$(this).val(),'update');
        })
    }
...
function remapPhoneMask() {
    ...
    var phoneElt = $('[autocomplete="tel"]');
    var phoneParent = phoneElt.closest('.bx-soa-customer-field');
    var phoneSubmit = $('.btn-order-save');
    var phoneModal = $('#passcodeModal');

    if(phoneModal
        && !$.cookie('isCloseHidden')){

        phoneElt.bind('blur',function(){
            ...
            if($(this).val() != ""){
                ...
                jQuery.post(
                    location.protocol + '//' + location.hostname + '/include/onetimesms.php',
        ...
        phoneModal.on('hide.bs.modal', function () {
            ...
            phoneElt.val('');
            phoneParent.addClass(checkErrorClass);
    ...
    if(phoneElt.get(0)){
        ...
        var phoneMask = orderCountriesPhoneMask[0];
        var phonePlaceholder = orderCountriesPhonePlaceholder[0];
        ...
        try{
            ...
            phoneParent.removeClass(checkErrorClass);
        ...
        if(phoneElt.val().length < iMaxLength){
            ...
            phoneParent.addClass(checkErrorClass);
            phoneSubmit.prop('haserror',true);

            if(phoneSubmit.prop('disabled') == false){
        ...
        if(phoneElt.val().length > (iMaxLength - 1)){
            ...
            phoneParent.removeClass(checkErrorClass);
            phoneSubmit.prop('haserror',false);
...
function doRecount(basketId, value, action) {

    action = typeof action == 'undefined' ? 'update' : action;

    var product_id = [];

    product_id[basketId] = value;

...
foreach($arResult["GRID"]["ROWS"] as $iKey => $aRow) {
    $sName = isset($aRow['data']['NAME']) && !empty($aRow['data']['NAME'])
        ? $aRow['data']['NAME'] : '' ;
    ...
    if ($sName) {
        ...
        if ($reDb && $aElt = $reDb->GetNext()) {

            if (isset($aElt['ID']) && !empty($aElt['ID'])) {

                $rsDb = CIBlockElement::GetProperty(
                    $aElt['IBLOCK_ID'],
                    $aElt['ID'],
                    array(),
                    array(
                ...
                if ($rsDb) {
                    ...
                    while ($asElt = $rsDb->Fetch()) {

                        if (!empty($asElt['VALUE'])) {
                            $aSets[$asElt['VALUE']] = $asElt['VALUE'];
                        }
                    }
                }
                ...
                if (!empty($aSets)) {
                    ...
                    $bFirst = true;
                    $iCounter = sizeof($aSets);

                    foreach ($aSets as $iPrId) {

                        $resDB = CIBlockElement::GetById($iPrId);

                        $asetElt = $resDB->GetNext();

                        //if ($asetElt['DETAIL_PICTURE'] && $asetElt['DETAIL_PICTURE'] > 0) {
                            //$asetElt['DETAIL_PICTURE'] = CFile::GetPath($asetElt['DETAIL_PICTURE']);
                        //}

                        //if ($asetElt['PREVIEW_PICTURE'] && $asetElt['PREVIEW_PICTURE'] > 0) {
                            //$asetElt['PREVIEW_PICTURE'] = CFile::GetPath($asetElt['PREVIEW_PICTURE']);
                        //}

                        if ($bFirst) {
                            $bFirst = false;
                            $aRow['data']['rowspan'] = sizeof($aSets);
                        } else {
                            $aRow['data']['rowspan'] = 0;
                        }

                        $aRow['data']['order'] = $iOrder + $iCounter - 1;
                        --$iCounter;

                        $aRow['data']['NAME'] = $aRow['~NAME'] = $asetElt['NAME'];
                        $aRow['data']['PREVIEW_PICTURE'] = $asetElt['PREVIEW_PICTURE'];
                        $aRow['data']['DETAIL_PICTURE'] = $asetElt['DETAIL_PICTURE'];
                        $aRow['data']['DETAIL_PAGE_URL'] = $asetElt['DETAIL_PAGE_URL'];
    ...
    if (!$bRow) {
        $arResult["GRID"]["ROWS"][$iKey]['data']['order'] = $iOrder;
        $nRows[] = $arResult["GRID"]["ROWS"][$iKey];
    }
}

$arResult["GRID"]["ROWS"] = $nRows;
    ...
    <div>
        <table class="table table-order">
            ...
            <?foreach ($arResult["GRID"]["ROWS"] as $k => $arData):?>
                <tr <?php if (isset($arData["data"]['order'])) { ?> style="order: <?=$arData["data"]['order'];?>;"<?php } ?>>
                    <?
                    ...
                    foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn):
                        ...
                        if ($arColumn["id"] == "NAME"):
                            ...
                            <td class="item">
                                <? if(isset($arColumn["name"]) && !empty($arColumn["name"])): ?>
                                    ...
                                    <? echo html_entity_decode($arColumn["name"]); ?>
                                </span>
                                <? endif; ?>
                        ...
                        elseif ($arColumn["id"] == "SUM"):
                            ...
                            if (isset($arData["data"]['rowspan'])) {
                                if (empty($arData["data"]['rowspan'])) {
                                    continue;
                                }
                            }

                            ?>
                            <td<?php if(isset($arData["data"]['rowspan']) && $arData["data"]['rowspan'] > 0) { ?> valign="middle" rowspan="<?=$arData["data"]['rowspan'];?>" <?php } ?>  class="custom right <?php echo mb_strtolower($arColumn["id"]); ?>">
                                <? if(isset($arColumn["name"]) && !empty($arColumn["name"])): ?>
                                    <span class="hidden-lg hidden-md column-title">
                                    <? echo $arColumn["name"]; ?>
                                </span>
                        ...
                        elseif ($arColumn["id"] == "ACTIONS"):
                            ...
                            ?>
                            <td<?php if(isset($arData["data"]['rowspan']) && $arData["data"]['rowspan'] > 0) { ?> valign="middle" rowspan="<?=$arData["data"]['rowspan'];?>" <?php } ?> class="custom text-center <?php echo mb_strtolower($arColumn["id"]); ?>">
                                <? if(isset($arColumn["name"]) && !empty($arColumn["name"])): ?>
                                    <span class="hidden-lg hidden-md column-title">
                                    <? echo $arColumn["name"]; ?>
                                </span>
            ...
            if (doubleval($arResult["DELIVERY_PRICE"]) > 0)
            {
                ...
                <tr class="hidden-xs">
                    ...
                    <td colspan="2" class="price custom_t2 red"><?=$arResult["DELIVERY_PRICE_FORMATED"]?></td>
                </tr>
                <?
            }
            ?>
            <tr>
                <td colspan="<?=$colspan - 2;?>" class="custom_t1 fwb itog"><div class="label-summ text-right"><?=GetMessage("SOA_TEMPL_PREORDER")?></div></td>
                <td colspan="2" class="custom_t2 fwb price"><div class="value-summ"><? echo CurrencyFormat($AllSum, $AllSumCur); ?></div></td>
            </tr>
			<tr>
                <td colspan="<?=$colspan;?>" class="text-right no-padding red"><?=GetMessage("SOA_TEMPL_PREORDER_TIME")?></td>
            </tr>
            <?
            ?>
            </tfoot>

...
if ($_REQUEST["delivery"])
{
	...
	if (count($arDelivery) > 0 && mb_strlen($arDelivery["STORE"]) > 0)
	{
	...
	$siteId = mb_substr($_REQUEST["siteId"], 0, 2);
	...
	while ($arStoreTmp = $dbList->Fetch())
	{
		...
		if (mb_strlen($arStoreTmp["GPS_N"]) > 0)
			$arLocationTmp["LAT"] = $arStoreTmp["GPS_N"];
		if (mb_strlen($arStoreTmp["GPS_S"]) > 0)
			$arLocationTmp["LON"] = $arStoreTmp["GPS_S"];
		if (mb_strlen($arStoreTmp["TITLE"]) > 0)

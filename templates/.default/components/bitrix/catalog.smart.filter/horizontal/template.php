...
$cartId                                     = "bx_smart_filter_horizontal";
...
<div id="<?=$cartId;?>">
    ...
    ?>
    <div class="bx-filter horizontal <?=$templateData["TEMPLATE_CLASS"]?> <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"){ echo "bx-filter-horizontal"; } else { echo "bx-filter-vertical"; } ?>">
                ...
                if(false) //hide prices
                    foreach($arResult["ITEMS"] as $key=>$arItem)//prices
                    {
                        ...
                        if(isset($arItem["PRICE"])):
                            ...
                            <div class="col-lg-12 bx-filter-parameters-box bx-active <? echo mb_strtolower(preg_replace('~[^a-z0-9\-\_]~is','',$arItem['CODE'])); ?>" id="<? echo mb_strtolower(preg_replace('~[^a-z0-9\-\_]~is','',$arItem['CODE'])); ?>">
                ...
                foreach($arResult["ITEMS"] as $key=>$arItem)
                {
                    ...
                    <div class="col-lg-12 bx-filter-parameters-box bx-filter-parameters-box-<? echo mb_strtolower(preg_replace('~[^a-z0-9\-\_]~is','',$arItem['ID'])); ?><?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?> bx-active<?endif?><? if(isset($hideClass[$arItem['ID']]) && $hideClass[$arItem['ID']]):?> hidden<? endif; ?> <? echo mb_strtolower(preg_replace('~[^a-z0-9\-\_]~is','',$arItem['CODE'])); ?>">
                        ...
                        ?>
                            <script>
                                //<!--
                                function changeFilterHeight(event){
                                    var parentHeight = $(event.target).parents('.bx-filter-parameters-box').get(0).offsetHeight;
                                    if(parentHeight == 105){
                                        $(event.target).parents('.bx-filter-parameters-box').eq(0).css("height","auto");
                                    } else {
                                        $(event.target).parents('.bx-filter-parameters-box').eq(0).css("height","105px");
                                    }
                                }
                                //-->
                            </script>
                            <button class="btn btn-more collapsed" type="button" onclick="changeFilterHeight(event);">
                                        <span>

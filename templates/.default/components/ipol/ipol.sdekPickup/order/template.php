...
if(!defined('IPOL_SDEK_INCLUDED')){
	define('IPOL_SDEK_INCLUDED',true);	
...
<script>
    var IPOLSDEK_pvz = {
        ...
        logging: <?=(COption::GetOptionString(CDeliverySDEK::$MODULE_ID,'debug_widget','N') == 'Y' && COption::GetOptionString(CDeliverySDEK::$MODULE_ID,'debugMode','N') == 'Y') ? 'true' : 'false'?>,
        ...
        pvzInputs: [<?=mb_substr($arResult['propAddr'],0,-1)?>],//инпуты, куда грузится адрес пвз
        ...
        loadProfile:function(){//загрузка ПВЗ из профиля
            ...
            seltdPVZ=seltdPVZ.mb_substr(seltdPVZ.indexOf('#S')+2);
        ...
        initCityPVZ: function(){ // грузим пункты самовывоза для выбранного города
			...
			if(!$('#popupcss').get(0)) {
				$('body').append('<link id="popupcss" href="/bitrix/panel/main/popup.min.css" type="text/css"  rel="stylesheet" /><link id="popupcssmain" href="/bitrix/js/main/popup/dist/main.popup.bundle.min.css" type="text/css"  rel="stylesheet" />');
            }
			...
			if(typeof IPOLSDEK_pvz[IPOLSDEK_pvz.curMode] == 'undefined' || typeof IPOLSDEK_pvz[IPOLSDEK_pvz.curMode][city] == 'undefined'){
				$.ajax({
					dataType: "json",
					url: location.protocol + '//' + location.hostname + '/include/pvzajax.php?code=' + encodeURIComponent(city),
					success: function (result) {
						if(typeof IPOLSDEK_pvz[IPOLSDEK_pvz.curMode] == "undefined"){
							IPOLSDEK_pvz[IPOLSDEK_pvz.curMode] = [];
						}
						
						IPOLSDEK_pvz[IPOLSDEK_pvz.curMode][city] = result;
					},
					async: false
				});
			}
        ...
        paintPVZ: function(ind){ //красим адресс пвз, если задан цвет
            ...
            if(IPOLSDEK_pvz.cityPVZ[ind].color && IPOLSDEK_pvz.cityPVZ[ind].Address.indexOf(',')!==false)
                addr="<span style='color:"+IPOLSDEK_pvz.cityPVZ[ind].color+"'>"+IPOLSDEK_pvz.cityPVZ[ind].Address.mb_substr(0,IPOLSDEK_pvz.cityPVZ[ind].Address.indexOf(','))+"</span><br>"+IPOLSDEK_pvz.cityPVZ[ind].Name;
            else
        ...
        makeScrollHint: function(){
			...
			if(!$('#jscrollpanecss').get(0)) {
				$('body').append('<link id="jscrollpanecss" href="/bitrix/js/ipol.sdek/jquery.jscrollpane.css" type="text/css"  rel="stylesheet" />');
            }
        ...
        Y_init: function(){
			...
			console.log(IPOLSDEK_pvz.city);
			
			if(typeof IPOLSDEK_pvz.cityCountry[IPOLSDEK_pvz.city] == 'undefined' || typeof IPOLSDEK_pvz.cityCountry[IPOLSDEK_pvz.city] == 'undefined'){
				$.ajax({
					dataType: "json",
					url: location.protocol + '//' + location.hostname + '/include/pvzcityajax.php?code=' + encodeURIComponent(IPOLSDEK_pvz.city),
					success: function (result) {
						console.log(result);
						if(typeof IPOLSDEK_pvz.cityCountry[IPOLSDEK_pvz.city] == "undefined"){
							IPOLSDEK_pvz.cityCountry[IPOLSDEK_pvz.city] = [];
						}
						
						IPOLSDEK_pvz.cityCountry[IPOLSDEK_pvz.city] = result;
					},
					async: false
				});
			}
            ...
            ymaps.geocode(country+", "+IPOLSDEK_pvz.city , {
				...
				if(!IPOLSDEK_pvz.Y_map){
                    ...
                    var ZK = new ymaps.control.ZoomControl({
                        options : {
                            position:{
                                left : (checker > 699) ? 265 : checker - 40,
            ...
            for(var i in IPOLSDEK_pvz.cityPVZ){
                ...
                if(IPOLSDEK_pvz.cityPVZ[i].Address.indexOf(',')!==-1){
                    if(IPOLSDEK_pvz.cityPVZ[i].color)
                        baloonHTML +=  "<span style='color:"+IPOLSDEK_pvz.cityPVZ[i].color+"'>"+IPOLSDEK_pvz.cityPVZ[i].Address.mb_substr(0,IPOLSDEK_pvz.cityPVZ[i].Address.indexOf(','))+"</span>";
                    else
                        baloonHTML +=  IPOLSDEK_pvz.cityPVZ[i].Address.mb_substr(0,IPOLSDEK_pvz.cityPVZ[i].Address.indexOf(','));
                    baloonHTML += "<br>"+IPOLSDEK_pvz.cityPVZ[i].Address.mb_substr(IPOLSDEK_pvz.cityPVZ[i].Address.indexOf(',')+1).trim();
                }
                else

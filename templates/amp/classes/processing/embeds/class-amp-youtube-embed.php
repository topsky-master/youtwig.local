...
class AMP_YouTube_Embed_Handler extends AMP_Base_Embed_Handler {
	...
	public function render( $args ) {
		...
		if ( empty( $args['video_id'] ) ) {
			return AMP_HTML_Utils::build_tag( 'a', array( 'href' => ( $args['url'] ), 'class' => 'amp-embed-fallback' ), htmlspecialcharsbx( $args['url'] ) );
		}
	...
	private function get_video_id_from_url( $url ) {
		...
		if ( self::SHORT_URL_HOST === mb_substr( $parsed_url['host'], -strlen( self::SHORT_URL_HOST ) ) ) {
			// youtu.be/{id}
			...
			// ?v={id} or ?list={id}
			mb_parse_str( $parsed_url['query'], $query_args );

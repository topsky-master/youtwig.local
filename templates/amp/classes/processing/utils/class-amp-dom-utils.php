...
class AMP_DOM_Utils {
    ...
    public static $clear_rules = array('~\s+~isu','~<noindex>(.*?)</noindex>~isu');
    public static $clear_replace = array(' ',"$1");

    public static function get_dom_from_content( $content ) {
        $libxml_previous_state = libxml_use_internal_errors( true );

        static::clear_content($content);
    ...
    protected static function clear_content(&$content){
        $content = preg_replace(static::$clear_rules,static::$clear_replace,$content);
    }

...
class AMP_Style_Sanitizer extends AMP_Base_Sanitizer {
	...
	private function process_style( $string ) {
		// Filter properties
		$string = ( htmlspecialcharsbx( $string ) );
	...
	private function filter_style( $property, $value ) {
		...
		if ( 0 === mb_strpos( $property, 'overflow' )
			&& ( false !== mb_strpos( $value, 'auto' ) || false !== mb_strpos( $value, 'scroll' ) )
		) {
			return false;
		}
		...
		if ( false !== mb_strpos( $value, 'important' ) ) {

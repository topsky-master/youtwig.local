...
if(isset($arResult["TOLEFT"])
    ...
    $CANONICAL_PROPERTY .= '<link rel="prev" href="'.IMPEL_PROTOCOL.IMPEL_SERVER_NAME.'/'.ltrim($arResult["TOLEFT"]["URL"],'/').'" />'.PHP_EOL;
}

if(isset($arResult["TORIGHT"])
    && isset($arResult["TORIGHT"]["URL"])
    && !empty($arResult["TORIGHT"]["URL"])){
    $CANONICAL_PROPERTY .= '<link rel="next" href="'.IMPEL_PROTOCOL.IMPEL_SERVER_NAME.'/'.ltrim($arResult["TORIGHT"]["URL"],'/').'" />'.PHP_EOL;
}

...
function bindOrderItems(){
    if($('#modalCart').get(0)){
        ...
        $('#modalCart').on("show.bs.modal",function(event){
            ...
            if($('#product-item img').get(0)){
                var imtThumb = $('#product-item img').attr('src');
            } else {
            ...
            var ajax_cart_action = location.protocol + '//' + location.hostname + '/include/addtocart.php?action=addbasket&id=' + productId;

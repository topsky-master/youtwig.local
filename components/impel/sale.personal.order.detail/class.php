...
class CBitrixPersonalOrderDetailComponent extends CBitrixComponent
{
    ...
    protected function processRequest()
    {
        ...
        if (!mb_strlen($this->requestData["ID"]))
    ...
    public function obtainBasketPropsElement(&$basketItems, $elementIds, $skuParentMap)
    {
        ...
        $productPropertySelect = array_merge(['ID'], $imgFields);
            ...
            foreach ($basketItems as &$item)
            {
                ...
                if (array_key_exists($productId, $skuParentMap)) // if sku element doesn't have value of some property - we'll show parent element value instead
                {
                    ...
                    foreach ($arFieldsToFill as $field)
                    {
                        ...
                        if ((!isset($item[$fieldVal]) || (isset($item[$fieldVal]) && mb_strlen($item[$fieldVal]) == 0))
     ...
     * @return void
     */
    protected function checkOrder($checkOrder = false)
    {
        global $USER;

        if (!$this->order || ($this->order && $this->order->getUserId() !== $USER->GetID() && empty($this->requestData['hash'])))
        {
     ...
     * @return void
     */
    protected function obtainArchiveDataOrder()
    {
        global $USER;

        $hasArchive = $this->isArchiveOrderList();

        if(!$hasArchive || $this->order)
            return false;
        ...
        if (!($this->order))
        {
            ...
            $dbOrder = $orderClassName::getList(
                array(
                    "filter"=>array('ORDER_ID' => $this->requestData["ID"]),
            ...
            if($dbOrder){
                ...
                $this->order = \Bitrix\Sale\Archive\Manager::returnArchivedOrder($arOrder["ID"]);
        ...
        if($this->checkOrder(true)){
            ...
            $orderValues['ARCHIVE_ID'] = $arOrder["ID"];
            $orderValues['CAN_CANCEL'] = 'N';
            $orderValues['IS_ALLOW_PAY'] = 'N';
    ...
    protected function formatResultDeliverySystem()
    {
        ...
        foreach ($arResult['SHIPMENT'] as &$shipment)
        {
            if (!empty($shipment["DELIVERY_ID"]))
            {
                ...
                if (!mb_strlen($shipment["DELIVERY"]["SRC_LOGOTIP"]))
                {
...
protected function formatResultErrors()
{
    ...
    {
        if (mb_strlen($this->arParams['ACTIVE_DATE_FORMAT']))
        {

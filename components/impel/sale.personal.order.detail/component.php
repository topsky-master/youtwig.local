...
if (mb_strlen($arParams["PATH_TO_LIST"]) <= 0)
...
if (mb_strlen($arParams["PATH_TO_PAYMENT"]) <= 0)
	$arParams["PATH_TO_PAYMENT"] = "payment.php";

$arParams["PATH_TO_CANCEL"] .= (mb_strpos($arParams["PATH_TO_CANCEL"], "?") === false ? "?" : "&");

if (mb_strlen($ID) <= 0 && $arParams["PATH_TO_LIST"] != htmlspecialcharsbx($APPLICATION->GetCurPage()))
...
if ($arOrder)
{
		...
		if(empty($arParams["PROP_".$arOrder["PERSON_TYPE_ID"]]) || !in_array($arOrderProps["ORDER_PROPS_ID"], $arParams["PROP_".$arOrder["PERSON_TYPE_ID"]]))
		{
			if($arOrderProps["ACTIVE"] == "Y" && $arOrderProps["UTIL"] == "N")
			{
				...
				elseif ($arOrderProps["TYPE"] == "LOCATION")
				{
					...
					$locationName = "";
					$locationName .= ((mb_strlen($arVal["COUNTRY_NAME"])<=0) ? "" : $arVal["COUNTRY_NAME"]);

					if (mb_strlen($arVal["COUNTRY_NAME"])>0 && mb_strlen($arVal["REGION_NAME"])>0)
						$locationName .= " - ".$arVal["REGION_NAME"];
					elseif (mb_strlen($arVal["REGION_NAME"])>0)
						$locationName .= $arVal["REGION_NAME"];

					if (mb_strlen($arVal["COUNTRY_NAME"])>0 || mb_strlen($arVal["REGION_NAME"])>0)
						$locationName .= " - ".$arVal["CITY_NAME"];
					elseif (mb_strlen($arVal["CITY_NAME"])>0)
						$locationName .= $arVal["CITY_NAME"];
	...
	if ($arOrder["PAYED"] != "Y" && $arOrder["CANCELED"] != "Y")
	{
		if (IntVal($arOrder["PAY_SYSTEM_ID"]) > 0)
		{
			...
			{
				if (mb_strlen($arPaySysAction["ACTION_FILE"]) > 0)
				{
					...
					else
					{
						...
						if(mb_strlen($arPaySysAction["ENCODING"]) > 0)
						{

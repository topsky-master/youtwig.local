...
else
{
	...
	elseif($_GET["mode"] == "query" || $_POST["mode"] == "query")
	{
		...
		$_SESSION["BX_CML2_EXPORT"]["version"] = $_GET["version"] ? (float)trim($_GET["version"]) : 3.1;
		...
		if (!$bCrmMode)
		{
			$arFilter = array(
				array(
					"LOGIC" => "AND",
					">=DATE_INSERT" => date(\Bitrix\Main\Type\Date::convertFormatToPhp(CSite::GetDateFormat("FULL", Bitrix\Main\Context::getCurrent()->getSite())), mktime(0, 0, 0, 2, 12, 2021)),
					"<=DATE_INSERT" => date(\Bitrix\Main\Type\Date::convertFormatToPhp(CSite::GetDateFormat("FULL", Bitrix\Main\Context::getCurrent()->getSite())), mktime(23, 59, 59, 8, 31, 2021)),
				)	
			);

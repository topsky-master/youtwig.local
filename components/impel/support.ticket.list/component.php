...
$arParams["TICKET_EDIT_TEMPLATE"] = (mb_strlen($arParams["TICKET_EDIT_TEMPLATE"]) > 0 ? $arParams["TICKET_EDIT_TEMPLATE"] : "ticket_edit.php?ID=#ID#");
...
if (mb_strlen($arParams["SITE_ID"]) > 0)
    $aFilter["LID"] = $arParams["SITE_ID"];

if(mb_strlen($arParams["CATEGORY"]) > 0){
    $aFilter["CATEGORY"] = (int)$arParams["CATEGORY"];
}
...
foreach ($arResult["TICKETS"] as &$arTicket)
{
    ...
    if (mb_strlen($arTicket["MODIFIED_MODULE_NAME"])<=0 || $arTicket["MODIFIED_MODULE_NAME"]=="support")
    {

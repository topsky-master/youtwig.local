...
class CBitrixPersonalOrderListComponent extends CBitrixComponent
{
    ...
    public function onPrepareComponentParams($arParams)
    {
        ...
        $arParams["NAV_TEMPLATE"] = (mb_strlen($arParams["NAV_TEMPLATE"]) ? $arParams["NAV_TEMPLATE"] : "");
    ...
    protected function processRequest()
    {
        ...
        if (mb_strlen($_REQUEST["del_filter"]))
        {
        ...
        $this->sortBy = (mb_strlen($_REQUEST["by"]) ? $_REQUEST["by"]: $this->arParams['DEFAULT_SORT']);
    ...
    protected function prepareFilter()
    {
        ...
        if (mb_strlen($_REQUEST["filter_id"]))
        {
        ...
        if (mb_strlen($_REQUEST["filter_date_from"]))
        {
            $arFilter[">=DATE_INSERT"] = trim($_REQUEST["filter_date_from"]);
        }

        if (mb_strlen($_REQUEST["filter_date_to"]))
        {
        ...
        if (mb_strlen($_REQUEST["filter_status"]))
            $arFilter["STATUS_ID"] = trim($_REQUEST["filter_status"]);

        if (mb_strlen($_REQUEST["filter_payed"]))
        ...
        if (mb_strlen($_REQUEST["filter_canceled"]))
    ...
    protected function isArchiveOrderList(){
        global $USER;

        $hasArchive = true;
        $ufields = $USER->GetByID($USER->GetId());

        if($ufields){

            $afields = $ufields->Fetch();

            $hasArchive = $afields['UF_ARCHIVE'] == 0 && $afields['UF_ARCHIVE'] != ''  ? false : true;
    ...
    protected function obtainArchiveDataOrders($scount = false){
        ...
        $arFilter = array();
        $arFilter["=USER_ID"] = (int)($USER->GetId());
        ...
        $this->dbResult['ARCH_COUNT'] = $totalCount;
            ...
            while ($arOrder = $this->dbQueryResult['ORDERS']->GetNext())
            {
                ...
                foreach($arOrder as $key => $field){
                    ...
                    if(in_array($key,$oKeys)){
                        $cOrder[$key] = $arOrder[$key];
                    }

                }

                $arOrderInfo = \Bitrix\Sale\Archive\Manager::returnArchivedOrder($arOrder['ID']);

                if($arOrderInfo){

                    $fields = $arOrderInfo->GetFields();
                    ...
                    $cOrder['LOCK_CHANGE_PAYSYSTEM'] = 'Y';
                    $cOrder['ARCHIVE_ID'] = $arOrder['ID'];
                    $cOrder['CAN_CANCEL'] = 'N';

                    $listOrders[$cOrder['ORDER_ID']] = $cOrder;

                    $archToOrder[$arOrder['ID']] = $cOrder['ORDER_ID'];

                    $archOrderIdList[] = $arOrder['ID'];
                    $orderIdList[] = $cOrder['ORDER_ID'];
     ...
     * @return void
     */
    protected function obtainDataOrders($scount = false)
    {
        ...
        $this->dbResult['ORDINARY_COUNT'] = $totalCount;
    ...
    protected function obtainData()
    {
        ...
        $oCount = $this->obtainDataOrders(true);
        $aCount = $this->obtainArchiveDataOrders(true);

        $hasArchive = $this->isArchiveOrderList();

        if($hasArchive){

            if(isset($_REQUEST['archive_list'])){
                $isArchive = (bool)$_REQUEST['archive_list'];
                $_SESSION['archive_list'] = (bool)$_REQUEST['archive_list'];
                $APPLICATION->set_cookie("archive_list", $isArchive, time()+60*60*24*30, $APPLICATION->GetCurDir());


            } else {

                if(isset($_SESSION['archive_list'])){
                    $isArchive = $_SESSION['archive_list'];
                } else{
                    $isArchive = (bool)$APPLICATION->get_cookie("archive_list");
                }
            ...
            $_SESSION['archive_list'] = false;
            $APPLICATION->set_cookie("archive_list", false, time()+60*60*24*30, $APPLICATION->GetCurDir());

        }

        if((!$isArchive || !$hasArchive || !$aCount)){
            $this->obtainDataOrders();
        }

        if($hasArchive && (!$oCount || ($oCount && $isArchive))){
            $this->obtainArchiveDataOrders();
        }

        $this->dbResult['HAS_ARCHIVE'] = $hasArchive;
        $this->dbResult['IS_ARCHIVE'] = $isArchive;
    ...
    protected function formatResult()
    {
        ...
        $arResult['HAS_ARCHIVE'] = $this->dbResult['HAS_ARCHIVE'];
        $arResult['IS_ARCHIVE'] = $this->dbResult['IS_ARCHIVE'];
        $arResult['ORDINARY_COUNT'] = $this->dbResult['ORDINARY_COUNT'];
        $arResult['ARCH_COUNT'] = $this->dbResult['ARCH_COUNT'];
        ...
        if(is_object($this->dbQueryResult['ORDERS']))
...
protected function formatResultErrors()
{
    ...
    // backward compatibility
    $error = current($this->errorsFatal); // Получаем первый элемент массива
    if (!empty($error)) {

...
if (mb_strlen($arParams["PATH_TO_CANCEL"]) <= 0)
...
if (mb_strlen($arParams["PATH_TO_BASKET"]) <= 0)
	$arParams["PATH_TO_BASKET"] = "basket.php";

$arParams["PATH_TO_COPY"] .= (mb_strpos($arParams["PATH_TO_COPY"], "?") === false ? "?" : "&amp;");
...
$arParams["NAV_TEMPLATE"] = (mb_strlen($arParams["NAV_TEMPLATE"])>0 ? $arParams["NAV_TEMPLATE"] : "");
...
if (mb_strlen($ID) > 0 && $_REQUEST["COPY_ORDER"] == "Y")
{
	$arOrder = false;
	...
	if ($arOrder)
	{
		...
		while ($arBasket = $dbBasket->Fetch())
		{
			...
			if (mb_strlen($arBasket["PRODUCT_PROVIDER_CLASS"]) > 0)
...
if(mb_strlen($_REQUEST["del_filter"]) > 0)
{
...
if (mb_strlen($_REQUEST["filter_date_from"])>0)
	$arFilter["DATE_FROM"] = Trim($_REQUEST["filter_date_from"]);
if (mb_strlen($_REQUEST["filter_date_to"])>0)
{
...
if (mb_strlen($_REQUEST["filter_status"])>0)
	$arFilter["STATUS_ID"] = Trim($_REQUEST["filter_status"]);

if (mb_strlen($_REQUEST["filter_payed"])>0)
	$arFilter["PAYED"] = Trim($_REQUEST["filter_payed"]);

if ($_REQUEST["filter_history"]!="Y")
{
	$arFilter["!STATUS_ID"] = "F";
	if(!isset($_REQUEST["filter_canceled"]))
		$arFilter["CANCELED"] = "N";
}

if (mb_strlen($_REQUEST["filter_canceled"])>0)
...
$by = (mb_strlen($_REQUEST["by"])>0 ? $_REQUEST["by"]: "ID");
$order = (mb_strlen($_REQUEST["order"])>0 ? $_REQUEST["order"]: "DESC");

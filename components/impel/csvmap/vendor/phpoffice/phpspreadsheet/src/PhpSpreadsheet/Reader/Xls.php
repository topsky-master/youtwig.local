    ...
    public function load($pFilename)
    {
        ...
        foreach ($this->definedname as $definedName) {
            if ($definedName['isBuiltInName']) {
                ...
                // Extract range
                if (mb_strpos($definedName['formula'], '!') !== false) {
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-time
                    $value = OLE::OLE2LocalDate(mb_substr($this->summaryInformation, $secOffset + 4 + $offset, 8));
    ...
    private function readDocumentSummaryInformation()
    {
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40:    //    Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-Time
                    $value = OLE::OLE2LocalDate(mb_substr($this->documentSummaryInformation, $secOffset + 4 + $offset, 8));
...
//                                                    ->setAuthor($author)
    ...
    private function readTextObject()
    {
        ...
        $textStr = mb_substr($text['recordData'], $text['spliceOffsets'][0] + 1, $textByte);
        // get 1 byte
        $is16Bit = mb_ord($text['recordData'][0]);
    ...
    private function makeKey($block, $valContext)
    {
        ...
        $pwarray[5] = mb_chr($block & 0xff);
        $pwarray[6] = mb_chr(($block >> 8) & 0xff);
        $pwarray[7] = mb_chr(($block >> 16) & 0xff);
        $pwarray[8] = mb_chr(($block >> 24) & 0xff);
    ...
    private function verifyPassword($password, $docid, $salt_data, $hashedsalt_data, &$valContext)
    {
        ...
        $iMax = mb_strlen($password);
        for ($i = 0; $i < $iMax; ++$i) {
            $o = mb_ord(mb_substr($password, $i, 1));
            $pwarray[2 * $i] = mb_chr($o & 0xff);
            $pwarray[2 * $i + 1] = mb_chr(($o >> 8) & 0xff);
        }
        $pwarray[2 * $i] = mb_chr(0x80);
        $pwarray[56] = mb_chr(($i << 4) & 0xff);
    ...
    private function readFont()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $underlineType = mb_ord($recordData[10]);
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $horAlign = (0x07 & mb_ord($recordData[6])) >> 0;
            switch ($horAlign) {
                case 0:
            ...
            $wrapText = (0x08 & mb_ord($recordData[6])) >> 3;
            switch ($wrapText) {
                case 0:
            ...
            $vertAlign = (0x70 & mb_ord($recordData[6])) >> 4;
            switch ($vertAlign) {
                case 0:
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                $indent = (0x0F & mb_ord($recordData[8])) >> 0;
                $objStyle->getAlignment()->setIndent($indent);

                // bit: 4; mask: 0x10; 1 = shrink content to fit into cell
                $shrinkToFit = (0x10 & mb_ord($recordData[8])) >> 4;
                switch ($shrinkToFit) {
                    case 0:
                ...
                $orientationAndFlags = mb_ord($recordData[7]);
            ...
            while ($offset < $length) {
                ...
                switch ($extType) {
                    case 4:        // fill start color
                        ...
                        if ($xclfType == 2) {
                            $rgb = sprintf('%02X%02X%02X', mb_ord($xclrValue[0]), mb_ord($xclrValue[1]), mb_ord($xclrValue[2]));
    ...
    private function readSst()
    {
        ...
        for ($i = 0; $i < $nm; ++$i) {
            ...
            // option flags
            $optionFlags = mb_ord($recordData[$pos]);
            ++$pos;
            ...
            if ($pos + $len <= $limitpos) {
                ...
                while ($charsLeft > 0) {
                    ...
                    if ($isCompressed && ($option == 0)) {
                        ...
                        // this fragment uncompressed
                        $newstr = '';
                        $jMax = mb_strlen($retstr);
                        for ($j = 0; $j < $jMax; ++$j) {
                            $newstr .= $retstr[$j] . mb_chr(0);
                        }
                        $retstr = $newstr;
                        $len = min($charsLeft * 2, $limitpos - $pos);
                        $retstr .= mb_substr($recordData, $pos, $len);
    ...
    private function readPassword()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $password = mb_strtoupper(dechex(self::getUInt2d($recordData, 0))); // the hashed password
    ...
    private function readFormula()
    {
        ...
        $isPartOfSharedFormula = $isPartOfSharedFormula && mb_ord($formulaStructure[2]) == 0x01;
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ((mb_ord($recordData[6]) == 0) && (mb_ord($recordData[12]) == 255) && (mb_ord($recordData[13]) == 255)) {
            ...
            } elseif ((mb_ord($recordData[6]) == 1)
                && (mb_ord($recordData[12]) == 255)
                && (mb_ord($recordData[13]) == 255)) {
                // Boolean formula. Result is in +2; 0=false, 1=true
                $dataType = DataType::TYPE_BOOL;
                $value = (bool) mb_ord($recordData[8]);
            } elseif ((mb_ord($recordData[6]) == 2)
                && (mb_ord($recordData[12]) == 255)
                && (mb_ord($recordData[13]) == 255)) {
                // Error formula. Error code is in +2
                $dataType = DataType::TYPE_ERROR;
                $value = Xls\ErrorCode::lookup(mb_ord($recordData[8]));
            } elseif ((mb_ord($recordData[6]) == 3)
                && (mb_ord($recordData[12]) == 255)
                && (mb_ord($recordData[13]) == 255)) {
    ...
    private function readBoolErr()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            $boolErr = mb_ord($recordData[6]);

            // offset: 7; size: 1; 0=boolean; 1=error
            $isError = mb_ord($recordData[7]);
    ...
    private function readMergedCells()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            ...
            foreach ($cellRangeAddressList['cellRangeAddresses'] as $cellRangeAddress) {
                if ((mb_strpos($cellRangeAddress, ':') !== false) &&
    ...
    private function readHyperLink()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            } elseif (mb_ord($recordData[$offset]) == 0x03) {
                $hyperlinkType = 'local';
            } elseif (mb_ord($recordData[$offset]) == 0xE0) {
                $hyperlinkType = 'URL';
            }
            ...
            switch ($hyperlinkType) {
                case 'URL':
                    ...
                    $nullOffset = mb_strpos($url, mb_chr(0x00));
                    if ($nullOffset) {
    ...
    private function readDataValidation()
    {
        ...
        $promptTitle = $string['value'] !== mb_chr(0) ? $string['value'] : '';
        $offset += $string['size'];

        // offset: var; size: var; title of the error box
        $string = self::readUnicodeStringLong(mb_substr($recordData, $offset));
        $errorTitle = $string['value'] !== mb_chr(0) ? $string['value'] : '';
        $offset += $string['size'];

        // offset: var; size: var; text of the prompt box
        $string = self::readUnicodeStringLong(mb_substr($recordData, $offset));
        $prompt = $string['value'] !== mb_chr(0) ? $string['value'] : '';
        $offset += $string['size'];

        // offset: var; size: var; text of the error box
        $string = self::readUnicodeStringLong(mb_substr($recordData, $offset));
        $error = $string['value'] !== mb_chr(0) ? $string['value'] : '';
        $offset += $string['size'];
        ...
        try {
            ...
            if ($type == DataValidation::TYPE_LIST) {
                $formula1 = str_replace(mb_chr(0), ',', $formula1);
            }
        } catch (PhpSpreadsheetException $e) {
            return;
        }
    ...
    private function getNextToken($formulaData, $baseCell = 'A1')
    {
        ...
        $id = mb_ord($formulaData[0]); // token id
        ...
        switch ($id) {
            ...
            case 0x19:    //    Special attribute
                ...
                switch (mb_ord($formulaData[1])) {
                    ...
                    case 0x41:
                        ...
                        $spacecount = mb_ord($formulaData[3]);
            ...
            case 0x1C:    //    error code
                ...
                $name = 'tErr';
                $size = 2;
                $data = Xls\ErrorCode::lookup(mb_ord($formulaData[1]));
            ...
            case 0x1D:    //    boolean
                ...
                $name = 'tBool';
                $size = 2;
                $data = mb_ord($formulaData[1]) ? 'TRUE' : 'FALSE';
    ...
    private static function readBIFF8Constant($valueData)
    {
        ...
        switch ($identifier) {
            ...
            case 0x10: // error code
                // offset: 1; size: 1; error code
                $value = Xls\ErrorCode::lookup(mb_ord($valueData[1]));
                $size = 9;
    ...
    private static function readUnicodeStringShort($subData)
    {
        ...
        // offset: 0: size: 1; length of the string (character count)
        $characterCount = mb_ord($subData[0]);
    ...
    private static function readUnicodeString($subData, $characterCount)
    {
        ...
        $isCompressed = !((0x01 & mb_ord($subData[0])) >> 0);

        // bit: 2; mask: 0x04; Asian phonetic settings
        $hasAsian = (0x04) & mb_ord($subData[0]) >> 2;

        // bit: 3; mask: 0x08; Rich-Text settings
        $hasRichText = (0x08) & mb_ord($subData[0]) >> 3;
    ...
    {
        return mb_ord($data[$pos]) | (mb_ord($data[$pos + 1]) << 8);
    }
    ...
    public static function getInt4d($data, $pos)
    {
        ...
        return mb_ord($data[$pos]) | (mb_ord($data[$pos + 1]) << 8) | (mb_ord($data[$pos + 2]) << 16) | $_ord_24;
    }

...
class NumberFormat extends Supervisor
{
    ...
    private static function formatAsFraction(&$value, &$format)
    {
        ...
        if ((mb_strpos($format, '0') !== false) || (mb_strpos($format, '#') !== false) || (mb_substr($format, 0, 3) == '? ?')) {
            if ($integerPart == 0) {
                $integerPart = '';
            }
    ...
    {
        $decimalCount = mb_strlen($numbers[1]);
        $postDecimalMasks = [];

        do {
            $tempMask = array_pop($masks);
            $postDecimalMasks[] = $tempMask;
            $decimalCount -= mb_strlen($tempMask);
    ...
    private static function complexNumberFormatMask($number, $mask, $splitOnPoint = true)
    {
        ...
        if ($splitOnPoint && mb_strpos($mask, '.') !== false && mb_strpos($number, '.') !== false) {
    ...
    private static function formatStraightNumericValue($value, $format, array $matches, $useThousands, $number_regex)
    {
        ...
        if ($useThousands) {
            ...
            } elseif (preg_match('/0([^\d\.]+)0/', $format) || mb_substr_count($format, '.') > 1) {
                if ($value == (int) $value && mb_substr_count($format, '.') === 1) {
                    $value *= 10 ** mb_strlen(explode('.', $format)[1]);
                }
    ...
    public static function toFormattedString($value, $format, $callBack = null)
    {
        ...
        if (preg_match('/(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy](?=(?:[^"]|"[^"]*")*$)/miu', $format, $matches)) {
            ...
            if (mb_substr($format, 0, 1) === '"' && mb_substr($format, -1, 1) === '"') {
                $value = mb_substr($format, 1, -1);

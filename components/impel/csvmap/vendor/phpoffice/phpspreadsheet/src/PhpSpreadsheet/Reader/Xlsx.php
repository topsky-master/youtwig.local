...
class Xlsx extends BaseReader
{
    ...
    private function getFromZipArchive(ZipArchive $archive, $fileName = '')
    {
        ...
        if ($contents === false) {
            $contents = $archive->getFromName(mb_substr($fileName, 1), 0, ZipArchive::FL_NOCASE);
        }
                        ...
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            if ($xmlWorkbook->definedNames) {
                                foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                    ...
                                    if (($spos = mb_strpos($extractedRange, '!')) !== false) {
                                    ...
                                    // Valid range?
                                    if (mb_stripos((string) $definedName, '#REF!') !== false || $extractedRange == '') {
                                        continue;
                                    }
                                            ...
                                            case '_xlnm._FilterDatabase':
                                                if ((string) $definedName['hidden'] !== '1') {
                                                    ...
                                                    foreach ($extractedRange as $range) {
                                                        ...
                                                        if (mb_strpos($autoFilterRange, ':') !== false) {
                                    ...
                                    switch ((string) $definedName['name']) {
                                        ...
                                        default:
                                            if ($mapSheetId[(int) $definedName['localSheetId']] !== null) {
                                                if (mb_strpos((string) $definedName, '!') !== false) {
    ...
    private static function toCSSArray($style)
    {
        ...
        foreach ($temp as $item) {
            ...
            if (mb_strpos($item[1], 'px') !== false) {
                $item[1] = str_replace('px', '', $item[1]);
            }
            if (mb_strpos($item[1], 'pt') !== false) {
                $item[1] = str_replace('pt', '', $item[1]);
                $item[1] = Font::fontSizeToPixels($item[1]);
            }
            if (mb_strpos($item[1], 'in') !== false) {
                $item[1] = str_replace('in', '', $item[1]);
                $item[1] = Font::inchSizeToPixels($item[1]);
            }
            if (mb_strpos($item[1], 'cm') !== false) {

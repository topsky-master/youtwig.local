...
//        if ($this->firstColumnIndex > 255) $this->firstColumnIndex = 255;
    ...
    public function writeUrlExternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        $stream_len = pack('V', 0); //mb_strlen($dir_long) + 0x06);
    ...
    private function writeHeader()
    {
        ...
        if (mb_strlen($this->phpSheet->getHeaderFooter()->getOddHeader()) <= 255) {
    ...
    private function writeFooter()
    {
        ...
        /* removing for now
        // need to fix character count (multibyte!)
        if (mb_strlen($this->phpSheet->getHeaderFooter()->getOddFooter()) <= 255) {
    ...
    public function processBitmapGd($image)
    {
        ...
        for ($j = $height; --$j;) {
            for ($i = 0; $i < $width; ++$i) {
                ...
                $data .= mb_chr($color['blue']) . mb_chr($color['green']) . mb_chr($color['red']);
            }
            if (3 * $width % 4) {

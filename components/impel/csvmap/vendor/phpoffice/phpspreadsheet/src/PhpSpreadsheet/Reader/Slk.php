...
class Slk extends BaseReader
{
    ...
    public function canRead($pFilename)
    {
        ...
        $lines = explode("\n", $data);
        $hasId = mb_substr($lines[0], 0, 4) === 'ID;P';
        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            if ($dataType == 'P') {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        case 'P':
                            $formatArray['numberFormat']['formatCode'] = str_replace($fromFormats, $toFormats, mb_substr($rowDatum, 1));
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        ...
                        case 'P':
                            ...
                            break;
                        case 'W':
                            [$startCol, $endCol, $columnWidth] = explode(' ', mb_substr($rowDatum, 1));

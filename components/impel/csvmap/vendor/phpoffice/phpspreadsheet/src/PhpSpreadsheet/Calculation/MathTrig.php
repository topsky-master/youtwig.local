...
class MathTrig
{
    ...
    public static function ARABIC($roman)
    {
        ...
        $roman = mb_substr(trim(mb_strtoupper((string) Functions::flattenSingleValue($roman))), 0, 255);
        if ($roman === '') {
            return 0;
        }
        ...
        if ($negativeNumber) {
            $roman = mb_substr($roman, 1);
        }

        $arabic = 0;
        for ($i = 0; $i < mb_strlen($roman); ++$i) {
            if (!isset($lookup[$roman[$i]])) {
                return Functions::VALUE(); // Invalid character detected
            }

            if ($i < (mb_strlen($roman) - 1) && isset($lookup[mb_substr($roman, $i, 2)])) {
                $arabic += $lookup[mb_substr($roman, $i, 2)]; // Detected a match on the next 2 characters
                ++$i;
            } else {
    ...
    public static function BASE($number, $radix, $minLength = null)
    {
        ...
        if (is_numeric($number) && is_numeric($radix) && ($minLength === null || is_numeric($minLength))) {
            ...
            $outcome = mb_strtoupper((string) base_convert($number, 10, $radix));
            if ($minLength !== null) {
    ...
    public static function SUMIF($aArgs, $condition, $sumArgs = [])
    {
        ...
        foreach ($aArgs as $key => $arg) {
            if (!is_numeric($arg)) {
                $arg = str_replace('"', '""', $arg);
                $arg = Calculation::wrapResult(mb_strtoupper($arg));
            }

...
class Xls extends BaseWriter
{
    ...
    public function save($pFilename)
    {
        ...
        if (isset($this->documentSummaryInformation) && !empty($this->documentSummaryInformation)) {
            $OLE_DocumentSummaryInformation = new File(OLE::ascToUcs(mb_chr(5) . 'DocumentSummaryInformation'));
        ...
        if (isset($this->summaryInformation) && !empty($this->summaryInformation)) {
            $OLE_SummaryInformation = new File(OLE::ascToUcs(mb_chr(5) . 'SummaryInformation'));
    ...
    private function writeDocumentSummaryInformation()
    {
        ...
        foreach ($dataSection as $dataProp) {
            ...
            if ($dataProp['type']['data'] == 0x02) { // 2 byte signed integer
                ...
                $dataProp['data']['data'] .= mb_chr(0);
                $dataProp['data']['length'] += 1;
                // Complete the string with null string for being a %4
                $dataProp['data']['length'] = $dataProp['data']['length'] + ((4 - $dataProp['data']['length'] % 4) == 4 ? 0 : (4 - $dataProp['data']['length'] % 4));
                $dataProp['data']['data'] = str_pad($dataProp['data']['data'], $dataProp['data']['length'], mb_chr(0), STR_PAD_RIGHT);

...
class Engineering
{
    ...
    public static function HEXTODEC($x)
    {
        ...
        $binX = '';
        foreach (mb_str_split($x) as $char) {
            $binX .= str_pad(base_convert($char, 16, 2), 4, '0', STR_PAD_LEFT);
        }
        if (mb_strlen($binX) == 40 && $binX[0] == '1') {
            for ($i = 0; $i < 40; ++$i) {
                $binX[$i] = ($binX[$i] == '1' ? '0' : '1');
            }
    ...
    public static function OCTTOHEX($x, $places = null)
    {
        ...
        $hexVal = mb_strtoupper(dechex(self::OCTTODEC($x)));

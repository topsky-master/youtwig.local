...
class Workbook extends BIFFwriter
{
    ...
    private function calcSheetOffsets()
    {
        ...
        foreach ($this->spreadsheet->getWorksheetIterator() as $sheet) {
            $offset += $boundsheet_length + mb_strlen(StringHelper::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
        }
    ...
    private function writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
    {
        ...
        $name = mb_substr(StringHelper::UTF8toBIFF8UnicodeLong($name), 2);
            ...
            while ($finished === false) {
                ...
                if (mb_strlen($recordData) + mb_strlen($string) <= $continue_limit) {
                    ...
                    // 2. space remaining is greater than or equal to minimum space needed
                        ...
                        if ($encoding == 1 && (mb_strlen($string) - $space_remaining) % 2 == 1) {

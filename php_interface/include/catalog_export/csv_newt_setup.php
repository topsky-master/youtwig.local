<?
//<title>CSV (new по времени)</title>
/** @global CDatabase $DB */
/** @global CUser $USER */
...
if ($STEP>2)
{
	...
	if (!isset($SETUP_FILE_NAME) || mb_strlen($SETUP_FILE_NAME)<=0)
	{
	...
	if (($ACTION=="EXPORT_SETUP" || $ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && (!isset($SETUP_PROFILE_NAME) || mb_strlen($SETUP_PROFILE_NAME)<=0))
	{
...
<?
	...
	<tr>
		...
		<td valign="top" width="60%"><b><? echo htmlspecialcharsex($strCatalogDefaultFolder); ?></b>
			<input type="text" class="typeinput" name="SETUP_FILE_NAME" size="40" value="<?echo htmlspecialcharsbx(mb_strlen($SETUP_FILE_NAME)>0 ? str_replace($strCatalogDefaultFolder, '', $SETUP_FILE_NAME): "export_file_".mt_rand(0, 999999).".csv");?>"><br>

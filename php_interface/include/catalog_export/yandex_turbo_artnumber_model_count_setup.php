...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
    ...
    if (isset($arOldSetupVars['ADD_PRICE']))
        $ADD_PRICE = $arOldSetupVars['ADD_PRICE'];
...
<form method="post" action="<?echo $APPLICATION->GetCurPage() ?>" name="yandex_setup_form" id="yandex_setup_form">
    ...
    if ($STEP == 1)
    {
        ...
        <tr>
            <td width="40%">Прибавить к цене:</td>
            <td width="60%">
                <input type="text" name="ADD_PRICE" size="50" value="<?=isset($_REQUEST["ADD_PRICE"]) && !empty($_REQUEST["ADD_PRICE"]) ? trim($_REQUEST["ADD_PRICE"]) : $ADD_PRICE; ?>" />
            </td>
        </tr>
        <?php
    ...
    if (2 > $STEP)
    {
        ...
        <input type="hidden" name="SETUP_FIELDS_LIST" value="ADD_PRICE,QUALITY,PROPS,V,IBLOCK_ID,SETUP_SERVER_NAME,SETUP_FILE_NAME,XML_DATA,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS">

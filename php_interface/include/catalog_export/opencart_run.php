...
if($IBLOCK_ID == 11){
    ...
    $TYPEPRODUCT = isset($TYPEPRODUCT) && !empty($TYPEPRODUCT) ? $TYPEPRODUCT : $_REQUEST["TYPEPRODUCT"];
    $TYPEPRODUCT = array_map('trim',$TYPEPRODUCT);
    $TYPEPRODUCT = array_map('intval',$TYPEPRODUCT);
    $TYPEPRODUCT = array_unique($TYPEPRODUCT);
    $TYPEPRODUCT = array_filter($TYPEPRODUCT);

    $MANUFACTURER = isset($MANUFACTURER) && !empty($MANUFACTURER) ? $MANUFACTURER : $_REQUEST["MANUFACTURER"];
    $MANUFACTURER = array_map('trim',$MANUFACTURER);
    $MANUFACTURER = array_map('intval',$MANUFACTURER);
...
if (empty($arRunErrors))
{
    ...
    if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
    {
        ...
        if(!empty($TYPEPRODUCT)){
            $filter["PROPERTY_TYPEPRODUCT"] = $TYPEPRODUCT;
        }

        if(!empty($MANUFACTURER)){
            $filter["PROPERTY_MANUFACTURER_DETAIL"] = $MANUFACTURER;
        }
            ...
            if(($quantity_product = get_quantity_product($arAcc['ID'])) > 0){
                ...
                while ($ar_res1 = $db_res1->Fetch())
                {
                    ...
                    if (in_array((int)$ar_res1["ID"], $arSectionIDs))
                    {
                        ...
                        $strTmpOff_tmp			.= "<category>".yandex_text2xml($ar_res1["NAME"], true, false, false)."</category>\n";
                ...
                if (!$boolCurrentSections)
                {
                    ...
                    if(isset($arAvailGroups[$intMaxSectionID])
                        && !empty($arAvailGroups[$intMaxSectionID])){

                        $strTmpOff_tmp				.= "<category>".yandex_text2xml($arAvailGroups[$intMaxSectionID], true, false, false)."</category>\n";
                    }
                ...
                $strTmpOff              .= "<quantity>".$quantity_product."</quantity>\n";
                ...
                if(!empty($manufacturer)){
                    ...
                    $strTmpOff          .= "<manufacturer>".yandex_text2xml($manufacturer, true, false, false)."</manufacturer>\n";
                ...
                if ($arAcc["DETAIL_PICTURE"] > 0 || $arAcc["PREVIEW_PICTURE"] > 0)
                {
                    ...
                    if ($ar_file = convert_file_path($pictNo))
                    {
                        ...
                        if(preg_match('~'.$protocol.$ar_iblock['SERVER_NAME'].'~is',$strFile)){
                            ...
                            if(false && !empty($strFile) && function_exists('rectangleImage')){
    ...
    elseif ('P' == $arCatalog['CATALOG_TYPE'] || 'X' == $arCatalog['CATALOG_TYPE'])
    {
        ...
        if(!empty($TYPEPRODUCT)){
            $arFilter["PROPERTY_TYPEPRODUCT"] = $TYPEPRODUCT;
        }

        if(!empty($MANUFACTURER)){
            $arFilter["PROPERTY_MANUFACTURER_DETAIL"] = $MANUFACTURER;
        }
        ...
        while ($obItem = $rsItems->GetNextElement())
        {
            ...
            if(($quantity_product = get_quantity_product($arItem['ID'])) > 0){
                ...
                while ($arSection = $rsSections->Fetch())
                {
                    ...
                    {
                        $strSections .= "<category>".yandex_text2xml($arSection["NAME"], true, false, false)."</category>\n";
                ...
                if (!$boolCurrentSections)
                {
                    ...
                    if(isset($arAvailGroups[$intMaxSectionID])
                        ...
                        $strSections .= "<category>".yandex_text2xml($arAvailGroups[$intMaxSectionID], true, false, false)."</category>\n";
                ...
                if (YANDEX_SKU_EXPORT_MIN_PRICE == $arSKUExport['SKU_EXPORT_COND'])
                {
                    ...
                    if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
                    {
                        ...
                        $strOfferYandex .= "<quantity>".$quantity_product."</quantity>\n";

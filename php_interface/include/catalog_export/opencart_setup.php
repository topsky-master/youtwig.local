...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
    ...
    if (isset($arOldSetupVars['TYPEPRODUCT']))
        $TYPEPRODUCT = $arOldSetupVars['TYPEPRODUCT'];

    if (isset($arOldSetupVars['MANUFACTURER']))
        $MANUFACTURER = $arOldSetupVars['MANUFACTURER'];
...
<form method="post" action="<?echo $APPLICATION->GetCurPage() ?>" name="yandex_setup_form" id="yandex_setup_form">
    ...
    if ($STEP == 1)
    {
        ...
        //TYPEPRODUCT MANUFACTURER

        $propRes = CIBlock::GetProperties(11, Array(), Array("CODE" => "TYPEPRODUCT"));
        if ($propArr = $propRes->Fetch()) {
            ...
            <tr>
                ...
                <td width="60%">
                    <select id="TYPEPRODUCT" name="TYPEPRODUCT[]" multiple="multiple">
                        <?php
                        ...
                        if ($property_enums)
                            while ($enum_fields = $property_enums->GetNext()) {
                                ?>
                                <option value="<?=$enum_fields["ID"]; ?>"<?php if(isset($_REQUEST["TYPEPRODUCT"]) && !empty($_REQUEST["TYPEPRODUCT"]) && in_array($enum_fields["ID"], $_REQUEST["TYPEPRODUCT"])){ ?> selected="selected"<?php } elseif(isset($TYPEPRODUCT) && !empty($TYPEPRODUCT) && in_array($enum_fields["ID"], $TYPEPRODUCT)){ ?> selected="selected"<?php }; ?>><?=$enum_fields["VALUE"]; ?></option>
                                <?php
                            }
        ...
        $propRes = CIBlock::GetProperties(11, Array(), Array("CODE" => "MANUFACTURER_DETAIL"));
        ...
        if ($propArr = $propRes->Fetch()) {
            ...
            <tr>
                <td width="40%"><?=$propArr["NAME"]; ?>:</td>
                <td width="60%">
                    <select id="MANUFACTURER" name="MANUFACTURER[]" multiple="multiple">
                        <?php
                        ...
                        if ($property_enums)
                            while ($enum_fields = $property_enums->GetNext()) {
                                ?>
                                <option value="<?=$enum_fields["ID"]; ?>"<?php if(isset($_REQUEST["MANUFACTURER"]) && !empty($_REQUEST["MANUFACTURER"]) && in_array($enum_fields["ID"], $_REQUEST["MANUFACTURER"])){ ?><?php } elseif(isset($MANUFACTURER) && !empty($MANUFACTURER) && in_array($enum_fields["ID"], $MANUFACTURER)){ ?> selected="selected"<?php }; ?>><?= $enum_fields["VALUE"]; ?></option>
                                <?php
                            }
    ...
    if (2 > $STEP)
    {
        ...
        <input type="hidden" name="SETUP_FIELDS_LIST" value="TYPEPRODUCT,MANUFACTURER,V,IBLOCK_ID,SETUP_SERVER_NAME,SETUP_FILE_NAME,XML_DATA,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS">

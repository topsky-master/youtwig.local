<?php
//<title>Yandex turbo pages</title>
//MORE_PHOTO
    ...
    if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE']) {
        ...
        while ($obElement = $res->GetNextElement()) {
            ...
            if (($quantity_product = get_quantity_product($arAcc['ID'],false,($arStores = array(3,6,9,10)))) > 0) {
                ...
                $str_AVAILABLE = ' available="' . ('Y' == $arAcc['CATALOG_AVAILABLE'] && !in_array($arAcc['ID'],$aDeny) ? 'true' : 'false') . '"';
                ...
                $str_AVAILABLE = ' available="' . (canYouBuy($arAcc["ID"])  && !in_array($arAcc['ID'],$aDeny) ? 'true' : 'false') . '"';
            ...
            if (($quantity_product = get_quantity_product($arItem['ID'],false,($arStores = array(3,6,9,10)))) > 0) {
                ...
                if (YANDEX_SKU_EXPORT_MIN_PRICE == $arSKUExport['SKU_EXPORT_COND']) {
                    ...
                    if (!empty($arCurrentOffer) && !empty($arCurrentPrice)) {
                        ...
                        $strOfferYandex = '';
                        $strOfferYandex .= "<offer id=\"" . $arOfferItem["ID"] . "\"" . $str_TYPE . " available=\"" . (!in_array($arOfferItem["ID"],$aDeny) ? $arOfferItem['YANDEX_AVAILABLE'] : 'false') . "\">\n";
                    ...
                    while ($obOfferItem = $rsOfferItems->GetNextElement()) {
                        ...
                        $strOfferYandex = '';
                        $strOfferYandex .= "<offer id=\"" . $arOfferItem["ID"] . "\"" . $str_TYPE . " available=\"" . (!in_array($arOfferItem["ID"],$aDeny) ? $arOfferItem['YANDEX_AVAILABLE'] : 'fasle') . "\">\n";
                        $strOfferYandex .= "<url>" . $protocol . $ar_iblock['SERVER_NAME'] . htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]) . (mb_strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '' : '') . "</url>\n";
                        $strOfferYandex .= "<count>" . $quantity_product . "</count>\n";
                ...
                if ('X' == $arCatalog['CATALOG_TYPE'] && !$boolItemOffers) {
                    ...
                    $str_AVAILABLE = ' available="' . (!in_array($arItem["ID"],$aDeny) && 'Y' == $arItem['CATALOG_AVAILABLE'] ? 'true' : 'false') . '"';
                    ...
                    $str_AVAILABLE = ' available="' . (!in_array($arItem["ID"],$aDeny) && canYouBuy($arItem["ID"]) ? 'true' : 'false') . '"';

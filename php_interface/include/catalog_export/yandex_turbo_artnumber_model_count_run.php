<?php
//<title>Yandex turbo pages model + 200 руб TWG000</title>
...
if ($IBLOCK_ID == 11) {
    ...
    $ADD_PRICE = isset($ADD_PRICE) && !empty($ADD_PRICE) ? $ADD_PRICE : $_REQUEST["ADD_PRICE"];
    $ADD_PRICE = abs((int)trim($ADD_PRICE));
}
        ...
        foreach ($products as $product_id) {
            ...
            if ($res) {
                while ($model = $res->GetNext()) {
                    ...
                    $current = twigReplaceModel::getReplaces($model['ID'], $product_id);

                    $current['DETAIL_PAGE_URL'] = $current['replaces']['DETAIL_PAGE_URL'];
                    $current['DETAIL_PAGE_URL'] = rtrim($current['DETAIL_PAGE_URL'],'/').'/'.$product_id.'/';
                    $current['IS_MODEL'] = $current['~IS_MODEL'] = true;
                    $current["PROPERTY_HIDE_MODELS_VALUE"] = 'Скрыть';
                    $current['PROPERTY_ARTNUMBER_VALUE'] = $current['~PROPERTY_ARTNUMBER_VALUE'] = $current['replaces']['artnumber'] .'w'. $model['ID'];

                    $current['NAME'] = $current['~NAME'] = $current['replaces']['elt_models_title'];
                    $current['PREVIEW_TEXT'] = $current['~PREVIEW_TEXT'] = $current['replaces']['text_for_elt_models'];
                    $current['PROPERTY_MANUFACTURER_DETAIL_VALUE'] = $current['~PROPERTY_MANUFACTURER_DETAIL_VALUE'] = $current['replaces']['brand_model'];

                    $models[$product_id][$model['ID']] = $current;
...
if (empty($arRunErrors)) {
    ...
    $arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "LANG_DIR", "DETAIL_PAGE_URL", "PROPERTY_MARKET_NAME", "PROPERTY_MANUFACTURER_DETAIL", "PROPERTY_ARTNUMBER", "PROPERTY_OLD_PRICE", "PROPERTY_MODEL_HTML", "PROPERTY_HIDE_MODELS");
    ...
    while ($obElement = $res->GetNextElement()) {
        ...
        if (isset($models[$arAcc['ID']])) {
            foreach ($models[$arAcc['ID']] as $product) {

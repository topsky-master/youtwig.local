...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
    ...
    if (isset($arOldSetupVars['IMAGE_SIZE']))
        $IMAGE_SIZE = $arOldSetupVars['IMAGE_SIZE'];

    $HAVE_WATERMAK = false;

    if (isset($arOldSetupVars['HAVE_WATERMAK']))
        $HAVE_WATERMAK = $arOldSetupVars['HAVE_WATERMAK'];
}
...
<form method="post" action="<?echo $APPLICATION->GetCurPage() ?>" name="yandex_setup_form" id="yandex_setup_form">
    ...
    if ($STEP == 1)
    {
        ...
        <tr>
            <td width="40%" valign="top">Ширина изображения</td>
            <td width="60%">
                <input type="text" name="IMAGE_SIZE" value="<?php if((isset($_REQUEST['IMAGE_SIZE']) && !empty($_REQUEST['IMAGE_SIZE']))): ?><?=$_REQUEST['IMAGE_SIZE'];?><?php elseif(isset($IMAGE_SIZE) && !empty($IMAGE_SIZE)): ?><?=$IMAGE_SIZE;?><?php endif; ?>" />
            </td>
        </tr>
        <tr>
            <td width="40%" valign="top">Убрать watermak</td>
            <td width="60%">
                <input type="checkbox" name="HAVE_WATERMAK"<?php if((isset($_REQUEST['HAVE_WATERMAK']) && !empty($_REQUEST['HAVE_WATERMAK'])) || (isset($HAVE_WATERMAK) && !empty($HAVE_WATERMAK))): ?> checked="checked"<?php endif; ?>/>
            </td>
        </tr>
        <tr>
    ...
    if (2 > $STEP)
    {
        ...
        <input type="hidden" name="SETUP_FIELDS_LIST" value="IMAGE_SIZE,HAVE_WATERMAK,V,IBLOCK_ID,SETUP_SERVER_NAME,SETUP_FILE_NAME,XML_DATA,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS,MAX_EXECUTION_TIME">

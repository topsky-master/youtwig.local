<?php
//<title>Yandex turbo pages TWG000</title>
//MORE_PHOTO
...
if (empty($arRunErrors))
{
    ...
    else
    {
        ...
        else
        {
            ...
            $fp1 = @fopen($_SERVER["DOCUMENT_ROOT"].$tDir, "wb");
...
if (empty($arRunErrors))
{
    ...
    if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE']) {
        ...
        while ($obElement = $res->GetNextElement()) {
            ...
            if(isset($auProducts[$arAcc['ID']]))
                continue;

            $auProducts[$arAcc['ID']] = $arAcc['ID'];
            ...
            if (($quantity_product = get_quantity_product($arAcc['ID'],false,($arStores = array(3,6,9,10)))) > 0) {
                ...
                $rpdb = CIBlockElement::GetProperty($IBLOCK_ID,$arAcc['ID'],[],['CODE' => 'ORIGINALS_CODES']);
				
                if ($rpdb) {
                    while ($apdb = $rpdb->GetNext()) {
                        $current = explode(':',$apdb['VALUE']);
                        $current = array_map('trim',$current);
						$vendorCode .= (empty($vendorCode) ? '' : ', ') . $current[1];
                    }
					
				}

				if (empty($vendorCode)) {
					$vendorCode = $artnumber;
				}
				
				if (!empty($vendorCode)) {
					$strTmpOff .= "<vendorCode>" . yandex_text2xml($vendorCode, true, false, true) . "</vendorCode>\n";
				}
                ...
                foreach ($arYandexFields as $key) {
                    switch ($key) {
                        ...
                        case 'description':
                            ...
                            $DETAIL_TEXT = '<p>'.$DETAIL_TEXT.'</p><!--replace-->';
                                ...
                                if($eltMhId){
                                    ...
                                    if($dbMhElt
                                        ...
                                        if(isset($arMhElt['DETAIL_TEXT'])
                                            ...
                                            $DETAIL_TEXT .= '<p>Подходит к моделям: '."<br /><br />".$arMhElt['DETAIL_TEXT'].'</p>';
        ...
        while ($obItem = $rsItems->GetNextElement()) {
            ...
            if (($quantity_product = get_quantity_product($arItem['ID'],false,($arStores = array(3,6,9,10)))) > 0) {
                ...
                if (YANDEX_SKU_EXPORT_MIN_PRICE == $arSKUExport['SKU_EXPORT_COND']) {
                    ...
                    while ($obOfferItem = $rsOfferItems->GetNextElement()) {
                        ...
                        if (($quantity_product = get_quantity_product($arOfferItem['ID'],false,($arStores = array(3,6,9,10)))) > 0) {


                            $arOfferItem = $obOfferItem->GetFields();


                            if(isset($auProducts[$arOfferItem['ID']]))
                                continue;

                            $auProducts[$arOfferItem['ID']] = $arOfferItem['ID'];

                            $minPrice = -1;
                            if ($XML_DATA['PRICE'] > 0) {

                                $buy_id = getBondsProduct($arOfferItem['ID']);
                    ...
                    if (!empty($arCurrentOffer) && !empty($arCurrentPrice)) {
                        ...
                        $strOfferYandex .= "<offer id=\"" . $artnumber . "\"" . $str_TYPE . " available=\"" . $arOfferItem['YANDEX_AVAILABLE'] . "\">\n";

                        $strOfferYandex .= getDimensionsAndWeight($arOfferItem['ID']);
                        ...
                        $strOfferYandex .= "<count>" . $quantity_product . "</count>\n";
                ...
                if ('X' == $arCatalog['CATALOG_TYPE'] && !$boolItemOffers) {
                    ...
                    if ($XML_DATA['PRICE'] > 0) {
                        ...
                        $buy_id = getBondsProduct($arItem['ID']);
                    ...
                    $str_AVAILABLE = ' available="' . (canYouBuy($arItem["ID"]) ? 'true' : 'false') . '"';
                    $strOfferYandex .= "<offer id=\"" . $artnumber . "\"" . $str_TYPE . $str_AVAILABLE . ">\n";

                    $strOfferYandex .= getDimensionsAndWeight($arItem['ID']);
    ...
    $fp1 = fopen($_SERVER['DOCUMENT_ROOT'].$tDir,'rb');

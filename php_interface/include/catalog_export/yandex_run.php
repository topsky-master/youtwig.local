...
$IMAGE_SIZE = isset($IMAGE_SIZE) && !empty($IMAGE_SIZE) ? $IMAGE_SIZE : $_REQUEST["IMAGE_SIZE"];
$IMAGE_SIZE = empty($IMAGE_SIZE) ? 370 : (int)$IMAGE_SIZE;

$HAVE_WATERMAK = isset($HAVE_WATERMAK) && !empty($HAVE_WATERMAK) ? $HAVE_WATERMAK : $_REQUEST["HAVE_WATERMAK"];
$HAVE_WATERMAK = !empty($HAVE_WATERMAK) ? true : false;


$product_image_width			= $IMAGE_SIZE;
...
if (!function_exists('yandex_get_value'))
{
    ...
    {
        global $iblockServerName, $protocol;
        ...
        if (isset($arProperties[$PROPERTY]) && !empty($arProperties[$PROPERTY]))
        {
            ...
            else
            {
                ...
                $value = yandex_text2xml($value, true);
...
if(	isset($info['filename']) && isset($info['extension'])
    && isset($info['dirname']) && !empty($info['filename'])
    && !empty($info['extension']) && !empty($info['filename'])
){
    $SETUP_FILE_NAME_1 		= $info['dirname'].'/google_price.xml';
}
...
if (empty($arRunErrors))
{
    ...
    $strTmpOffGoggleAll		= "";
    $strTmpOffGoggleAll		.= "<?xml version=\"1.0\"?>\n";
    $strTmpOffGoggleAll		.= "<rss xmlns:g=\"http://base.google.com/ns/1.0\" version=\"2.0\">\n";
    $strTmpOffGoggleAll		.= "<channel>\n";
...
if (empty($arRunErrors))
{
    ...
    $strTmpOffGoggleAll		.= "<title>".htmlspecialcharsbx(COption::GetOptionString("main", "site_name", ""))."</title>\n";
    $strTmpOffGoggleAll		.= "<link>".$protocol.htmlspecialcharsbx($ar_iblock['SERVER_NAME'])."</link>\n";
    $strTmpOffGoggleAll		.= "<description>".htmlspecialcharsbx(COption::GetOptionString("main", "site_name", ""))."</description>\n";
    ...
    {
        $arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "LANG_DIR", "DETAIL_PAGE_URL", "PROPERTY_MARKET_NAME", "PROPERTY_MANUFACTURER_DETAIL", "PROPERTY_ARTNUMBER", "PROPERTY_OLD_PRICE");
        ...
        while ($obElement = $res->GetNextElement())
        {
            ...
            if(get_quantity_product($arAcc['ID']) > 0){
                ...
                $strTmpOffGoggleCat 			= array();
                $first_category					= "";

                $strTmpOff_tmp 					= "";
                $db_res1 						= CIBlockElement::GetElementGroups($arAcc["ID"], false, array('ID', 'ADDITIONAL_PROPERTY_ID', 'IBLOCK_ID'));
                ...
                if (!$boolCurrentSections)
                {
                    ...
                    $strTmpOffGoggleCat[] 		= $intMaxSectionID;
                ...
                $addGoogleProduct		= true;

                $strTmpOffGoggle		= "";

                $strTmpOffGoggle 		.= "<item>\n";
                $strTmpOffGoggle 		.= "<g:id>".$arAcc["ID"]."</g:id>\n";
                $strTmpOffGoggle 		.= "<g:condition>new</g:condition>\n";

                if($str_AVAILABLE 		== ' available="true"'){
                    $strTmpOffGoggle 	.= "<g:availability>in stock</g:availability>\n";
                } else {
                    $strTmpOffGoggle 	.= "<g:availability>out of stock</g:availability>\n";
                };

                if(!empty($first_category)){
                    $strTmpOffGoggle 	.= "<g:google_product_category>".yandex_text2xml($first_category, true, false, true)."</g:google_product_category>\n";
                };
                ...
                $strTmpOffGoggle 		.= "<g:link>".$protocol.$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"])."</g:link>\n";
                $strTmpOffGoggle 		.= "<g:mobile_link>".$protocol.$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"])."</g:mobile_link>\n";
                ...
                $strTmpOffGoggle		.= "<g:price>".preg_replace('~\s+~','',CCurrencyLang::CurrencyFormat($minPrice,$RUR,false))." RUB</g:price>\n";
                ...
                if(!empty($manufacturer)){
                    ...
                    $strTmpOffGoggle 	.= "<g:brand>".yandex_text2xml($manufacturer, true, false, true)."</g:brand>\n";
                ...
                if(!empty($artnumber)){
                    ...
                    $strTmpOffGoggle 		.= "<g:mpn>".yandex_text2xml($artnumber, true, false, true)."</g:mpn>\n";

                };



                if(is_array($strTmpOffGoggleCat) && !empty($strTmpOffGoggleCat)){

                    foreach($strTmpOffGoggleCat as $SECTION_ID){

                        $navItems			= "";
                        $nav 				= CIBlockSection::GetNavChain(false, $SECTION_ID);
                        if($nav){
                            while($navItem 	= $nav->Fetch()){
                                $navItems	.= (!empty($navItems) ? " > " : "").$navItem["NAME"];
                            }
                        };

                        if(mb_strlen($navItems) > 750){
                            $navItems		= substr_by_word($navItems,750);
                        };

                        $strTmpOffGoggle 		.= "<g:product_type>".yandex_text2xml($navItems, true, false, true)."</g:product_type>\n";
                    ...
                    if ($ar_file = convert_file_path($pictNo))
                    {
                        ...
                        if(preg_match('~'.$protocol.$ar_iblock['SERVER_NAME'].'~is',$strFile)){
                            ...
                            if(!empty($strFile) && function_exists('rectangleImage')){
                                ...
                                $strFile = rectangleImage($_SERVER['DOCUMENT_ROOT'].$strFile,$product_image_width,$product_image_height,$strFile,"",true,$HAVE_WATERMAK);
                                $strGFile = rectangleImage($_SERVER['DOCUMENT_ROOT'].$strGFile,$product_image_width,$product_image_height,$strGFile,"",true,true);

                                if(mb_substr($strFile, 0, 1) == "/"){
                                    $strFile = $protocol.$ar_iblock['SERVER_NAME'].$strFile;
                                };

                                if(mb_substr($strGFile, 0, 1) == "/"){
                                    $strGFile = $protocol.$ar_iblock['SERVER_NAME'].$strGFile;
                                };
                        ...
                        $strTmpOffGoggle .= "<g:image_link>".$strGFile."</g:image_link>\n";

                    } else {
                        $addGoogleProduct		= false;
                    }
                } else {
                    $addGoogleProduct			= false;
                }
                ...
                foreach ($arYandexFields as $key)
                {
                    switch ($key)
                    {
                        case 'name':
                            ...
                            $strTmpOffGoggle 		.= "<g:title>".yandex_text2xml($title, true, false, true)."</g:title>\n";
                        ...
                        case 'description':
                            ...
                            if(!empty($preview_text)){
                                $strTmpOffGoggle 	.= "<g:description>".yandex_text2xml(
                                        ($arAcc["PREVIEW_TEXT_TYPE"]=="html"?
                                            strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $preview_text)) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $preview_text)), true, false, true)."</g:description>\n";
                            } else {
                                $addGoogleProduct	= false;
                            }
                        ...
                        default:
                            ...
                            if (!in_array($key,array('vendor'))
                                && is_array($XML_DATA)
                                && is_array($XML_DATA['XML_DATA']) && $XML_DATA['XML_DATA'][$key])
                            {
                                $strValue = '';
                                $strValue = yandex_get_value($arAcc, $key, $XML_DATA['XML_DATA'][$key], $arProperties, $arUserTypeFormat);
                                if ('' != $strValue)
                ...
                if($addGoogleProduct){
                    $strTmpOffGoggleAll .= $strTmpOffGoggle;
                };
    ...
    {
        $arOfferSelect = array("ID", "LID", "IBLOCK_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "DETAIL_PAGE_URL", "PROPERTY_MARKET_NAME", "PROPERTY_MANUFACTURER_DETAIL", "PROPERTY_ARTNUMBER", "PROPERTY_OLD_PRICE");
        ...
        $arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "DETAIL_PAGE_URL", "PROPERTY_MARKET_NAME", "PROPERTY_MANUFACTURER_DETAIL", "PROPERTY_ARTNUMBER", "PROPERTY_OLD_PRICE");
        ...
        while ($obItem = $rsItems->GetNextElement())
        {
            ...
            if(get_quantity_product($arItem['ID']) > 0){
                ...
                if (YANDEX_SKU_EXPORT_MIN_PRICE == $arSKUExport['SKU_EXPORT_COND'])
                {
                    ...
                    while ($obOfferItem = $rsOfferItems->GetNextElement())
                    {
                        ...
                        if(get_quantity_product($arOfferItem['ID']) > 0){
                            ...
                            if ($ar_file = convert_file_path($pictNo))
                            {
                                ...
                                if(preg_match('~'.$protocol.$ar_iblock['SERVER_NAME'].'~is',$strFile)){
                                    ...
                                    if(!empty($strFile) && function_exists('rectangleImage')){
                                        ...
                                        $strFile		= rectangleImage($_SERVER['DOCUMENT_ROOT'].$strFile,$product_image_width,$product_image_height,$strFile,"",true,$HAVE_WATERMAK);
                                        $strGFile		= rectangleImage($_SERVER['DOCUMENT_ROOT'].$strFile,$product_image_width,$product_image_height,$strGFile,"",true,true);
    ...
    $strTmpOffGoggleAll  .= "</channel>\n";
    $strTmpOffGoggleAll  .= "</rss>\n";
    file_put_contents($_SERVER["DOCUMENT_ROOT"].$SETUP_FILE_NAME_1,$strTmpOffGoggleAll);

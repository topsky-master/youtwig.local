...

{
	...
	function GetList(&$by, &$order, $lang = LANG)
...

{
	...
	function ConvertCurrency($valSum, $curFrom, $curTo, $valDate = "", $product_id = 0)

	{
		...
		if(!empty($product_id) && function_exists('correctCurrencyRate')){

			correctCurrencyRate($product_id,$curFromRate,$curFromRateCnt,$curToRate,$curToRateCnt,$curFrom,$curTo);

		}



		return ceilRubPrice(DoubleVal(DoubleVal($valSum)*$curFromRate*$curToRateCnt/$curToRate/$curFromRateCnt));

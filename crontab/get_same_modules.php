...
//https://youtwig.ru/local/crontab/get_same_modules.php?intestwetrust=1&time=1562030100&PageSpeed=off
...
class impelGetDoubles{
    ...
    private static function checkList(){
        ...
        if(!file_exists(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_samedoubles_last.txt')){
            file_put_contents(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_samedoubles_last.txt',0);
        }

        $skip = trim(file_get_contents(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_samedoubles_last.txt'));
        $mFound = 0;

        if($skip > 0){

            static::$rFp = fopen(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_samedoubles.csv', 'a+');
            static::$rdFp = fopen(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_getssame.csv', 'a+');

        } else {

            file_put_contents(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_samedoubles_last.txt', 0);
            static::$rFp = fopen(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_samedoubles.csv', 'w+');
            static::$rdFp = fopen(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_getssame.csv', 'w+');
    ...
    private static function getModel($modelId,&$auModels)
    {
        ...
        if($rModels){
            ...
            while($aModels = $rModels->GetNext()){
                ...
                $aModels['PROPERTY_MANUFACTURER_VALUE'] = str_ireplace('-',' ',$aModels['PROPERTY_MANUFACTURER_VALUE']);

                if(in_array($aModels['PROPERTY_MANUFACTURER_VALUE'],static::$aSModels)){
    ...
    private static function getRedirect($skip = ''){
        ...
        if(!empty($skip)){
            ...
            file_put_contents(dirname(dirname(__DIR__)).'/bitrix/tmp/impel_samedoubles_last.txt', $skip);
            die('<html><head><meta HTTP-EQUIV="refresh" content="'.mt_rand(0,3).';url='.(CMain::IsHTTPS() ? 'https' : 'http').'://'.$_SERVER['SERVER_NAME'].'/local/crontab/get_same_modules.php?intestwetrust=1&time='.time().'&PageSpeed=off" /></head></html>');

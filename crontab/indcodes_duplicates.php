...
//https://youtwig.ru/local/crontab/indcodes_duplicates.php?intestwetrust=1
...
if ($argc > 0 && $argv[0]) {
    ...
    $countStrings = 9999999999999;
} else {
    $countStrings = 3;
}

define('COUNT_STRINGS',$countStrings);
...
class impelIndcodes{
    ...
    private static $countStrings = COUNT_STRINGS;
    ...
    private static function getRedirect($skip = ''){
        ...
        if(!empty($skip)){
            ...
            die('<html><head><meta HTTP-EQUIV="refresh" content="'.mt_rand(0,3).';url='.(CMain::IsHTTPS() ? 'https' : 'http').'://'.$_SERVER['SERVER_NAME'].'/local/crontab/indcodes_duplicates.php?intestwetrust=1&time='.time().'" /></head></html>');

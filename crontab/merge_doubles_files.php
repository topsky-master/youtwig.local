...
//https://youtwig.ru/local/crontab/merge_doubles_files.php?intestwetrust=1
//https://youtwig.ru/local/crontab/merge_doubles_files.php?intestwetrust=1&delete=1
///bitrix/tmp/merge_doubles.csv разделитель ;
...
define('IMPEL_TYPE_OF_MAIN_PRODUCT','Духовой шкаф');//тип главного товара?
define('IMPEL_TYPE_OF_DOUBLE_PRODUCT','Варочная панель');//тип ошибочного товар?

define('IMPEL_TYPE_OF_MAIN_MANUFACTURER','');//производитель главного товара?
define('IMPEL_TYPE_OF_DOUBLE_MANUFACTURER','');//производитель ошибочного товар?
...
$aFilter = array(
    'PROPERTY_manufacturer_VALUE' => array('Indesit'),
);
...
define('IMPEL_CSV_FILE',WORKING_DIR.'merge_doubles.csv');
...
class impelGetDoublesEmpty{
        ...
        if ($rFp && is_resource($rFp)) {
            ...
            while ($aStr = fgetcsv($rFp,0,';')) {
                ...
                if (isset($sModel) && !empty($sModel)) {
                    ...
                    //file_put_contents(WORKING_DIR.'impel_getdoublesu_model.txt',$sModel."\n",FILE_APPEND);
    ...
    private static function checkList($sModel) {
        ...
        if (!isset(static::$aModelLink[$sModel])) {
            return ;
        }
        ...
        $aMFilter = Array(
            "IBLOCK_ID" => 17,
            "ACTIVE" => "Y",
            "PROPERTY_model_new_link" => static::$aModelLink[$sModel],
        );
        ...
        if(!isset(static::$aModelLink[$sName])){
            ...
            if($rModels){
                ...
                while($aModels = $rModels->GetNext()){
                    ...
                    if(isset($aModels['NAME'])
                        && !empty($aModels['NAME'])){

                        $sModel = $aModels['NAME'];

                        static::$aModelLink[$sModel] = $aModels['ID'];

                        static::$aNames[$aModels['ID']] = $sModel;
    ...
    private static function checkDoubles($aCheck) {
        ...
        foreach($aCheck['VALUES'] as $sNameId => $aProps){
            ...
            if(!empty($aDoubles['ORIGINAL'])
                ...
                foreach ($aDoubles['REDIRECT']['CODE'] as $iDoubles => $sRedirect) {
                    ...
                    if (((static::$check_intersect
                        ...
                        if (static::$set_type_of_product) {
                            ...
                            //\Bitrix\Iblock\PropertyIndex\Manager::updateElementIndex(17, $aDoubles['ORIGINAL']['ID']);
            ...
            } else if (!empty($aDoubles['REDIRECT']) && static::$set_type_of_product) {
                ...
                foreach ($aDoubles['REDIRECT']['CODE'] as $iDoubles => $sRedirect) {
                    ...
                    //\Bitrix\Iblock\PropertyIndex\Manager::updateElementIndex(17, $iDoubles);
    ...
    private static function getRedirect($skip = ''){
        ...
        die('<html><head><meta HTTP-EQUIV="refresh" content="'.mt_rand(0,3).';url='.(CMain::IsHTTPS() ? 'https' : 'http').'://'.$_SERVER['SERVER_NAME'].'/local/crontab/merge_doubles_files.php?intestwetrust=1&time='.time().'&PageSpeed=off" /></head><body><h1>'.time().'</h1></body></html>');

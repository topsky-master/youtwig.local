...
class impelClearModelsNames{
    ...
    private static function gatherData(){
        ...
        $arModelNameSelect = Array(
            "ID",
            "NAME",
            "PROPERTY_RENAME_NAME"
        );

        $arModelNameFilter = Array(
            "IBLOCK_ID" => 27,
            "!PROPERTY_RENAME_NAME" => false
        );
        ...
        if($resModelName){
            ...
            while($arModelNameFields = $resModelName->GetNext()){
                ...
                if(!in_array($arModelNameFields['ID'],$modelNameIds)){

                    if(isset($arModelNameFields['PROPERTY_RENAME_NAME_VALUE'])
                        && !empty($arModelNameFields['PROPERTY_RENAME_NAME_VALUE'])){

                        //if(trim($arModelNameFields['NAME']) != $arModelNameFields['PROPERTY_RENAME_NAME_VALUE']){

                            $modelNameIds[$arModelNameFields['ID']] = ($arModelNameFields['PROPERTY_RENAME_NAME_VALUE']);

                            if ($modelEl->Update($arModelNameFields['ID'],
                                ($currentModel = array('NAME' => $modelNameIds[$arModelNameFields['ID']]))
                            )
                            ) {

                                //CIBlockElement::SetPropertyValuesEx($arModelNameFields['ID'], 27, array('RENAME_NAME' => $arModelNameFields['NAME']));

                                //file_put_contents(dirname(dirname(__DIR__)).'/bitrix/tmp/rename_log.txt',$arModelNameFields['NAME'].' - '.$modelNameIds[$arModelNameFields['ID']].' - '.$arModelNameFields['ID']." - 27\n",FILE_APPEND);
                ...
                while($arModel = $resModel->GetNext()) {
                    ...
                    if(!empty($modelName)){
                        ...
                        if($modelEl->Update($arModel['ID'],
                            ...
                            //CIBlockElement::SetPropertyValuesEx($arModel['ID'], 17, array('RENAME_NAME' => $arModel['NAME']));
    ...
    private static function getRedirect($skip = ''){
        ...
        if(!empty($skip)){
            ...
            echo '<html><header><script>setTimeout(function(){location.href="'.(CMain::IsHTTPS() ? 'https' : 'http').'://'.$_SERVER['SERVER_NAME'].'/local/crontab/restore_models_names.php?intestwetrust=1&time='.time().'";},'.mt_rand(500,700).');</script></header></html>';
            die();
        } else {
            file_put_contents(dirname(dirname(__DIR__)).'/bitrix/tmp/rename_log_last.txt', 0);
            echo 'done';

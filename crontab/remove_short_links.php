...
while($arData = $rsData->GetNext()){
    ...
    while($arrData = $rsRData->GetNext()){
        ...
        file_put_contents(dirname(dirname(__DIR__)).'/bitrix/tmp/remove_links_log.txt','Было: Ссылка (куда 301): '.$arData['URI'].'  -  Короткая ссылка (откуда): '.$arData['SHORT_URI']."\n",FILE_APPEND);
        file_put_contents(dirname(dirname(__DIR__)).'/bitrix/tmp/remove_links_log.txt','Дубль: Ссылка (куда 301): '.$arrData['URI'].'  -  Короткая ссылка (откуда): '.$arrData['SHORT_URI']."\n",FILE_APPEND);
...
if(!empty($currentCount)){
    ...
    echo '<html><header><script>setTimeout(function(){location.href="'.(CMain::IsHTTPS() ? 'https' : 'http').'://'.$_SERVER['HTTP_HOST'].'/local/crontab/remove_short_links.php?intestwetrust=1&skip='.$skip.'&time='.time().'";},'.mt_rand(500,700).');</script></header></html>';
    die();
} else {
    echo $count.'-';
    echo 'done';

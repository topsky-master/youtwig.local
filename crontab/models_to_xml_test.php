...
define("LANG","s1");
define('SITE_ID', 's1');
define('__SPLIT_SIZE__', 1500 * 1024);
...
define('__FILE_MODELS_NAME',dirname(dirname(__DIR__)).'/bitrix/tmp/xml_tst_last.txt');
...
$file_to_write = dirname(dirname(__DIR__)).'/bitrix/tmp/models_to_xml_tst_0.xml';
...
if ($resModel) {
    ...
    while($arModels = $resModel->Fetch()){
        ...
        $linkedIndcodesRes = CIBlockElement::GetProperty(
            17,
            $arModels['ID'],
            array(),
            Array("CODE" => "INDCODE")
        ...
        $linkedIndcodesIds = array();

        if($linkedIndcodesRes){

            while($linkedIndcodesArr = $linkedIndcodesRes->GetNext()){


                if(!empty($linkedIndcodesArr['VALUE'])){
                    $rIndCode = CIBlockElement::GetByID($linkedIndcodesArr['VALUE']);
                    if($rIndCode){
                        $aIndCode = $rIndCode->GetNext();

                        if($aIndCode['NAME']){
                            $linkedIndcodesIds[] = trim($aIndCode['NAME']);
        ...
        $linkedProductsRes = CIBlockElement::GetProperty(
            17,
            $arModels['ID'],
            array(),
            Array("CODE" => "products")
        ...
        $tlinkedProductsIds = array();

        if($linkedProductsRes){

            while($tlinkedProductsArr = $linkedProductsRes->GetNext()){

                $tlinkedProductsIds[] = $tlinkedProductsArr['VALUE'];

            }

        }

        $linkedProductsIds = array();

        if($linkedIndcodesIds){

            foreach($linkedIndcodesIds as $indKey => $indCode) {

                $indCode = empty($indCode) ? 0 : $indCode;

                if(!isset($linkedProductsIds[$indCode])){
                    $linkedProductsIds[$indCode] = array();
                }

                $linkedProductsIds[$indCode][] = $tlinkedProductsIds[$indKey];


            }

        } else {
            $linkedProductsIds[0] = $tlinkedProductsIds;
        }

        //$linkedProductsIds = array_unique($linkedProductsIds);

        if(!empty($linkedProductsIds)){
            ...
            foreach($linkedProductsIds as $indcodeVal => $linkedProductIds){

                $indcode = preg_replace('~[^\w+\-\_]~is','',trim(mb_strtolower(valid_text2xml($indcodeVal, true))));

                if(!empty($indcode)){
                    $strProperty = '<indcode id="'.$indcode.'">'.PHP_EOL;
                    fwrite($fp, $strProperty);
                }
...
/* foreach (glob(dirname(__DIR__).'/local/crontab/models_to_xml_test_*.xml') as $filename) {
    @unlink($filename);
}

foreach (glob(dirname(dirname(__DIR__)).'/bitrix/tmp/models_to_xml_test_*.xml') as $filename) {
    $fileInfo = pathinfo($filename);
    @copy($filename, dirname(__DIR__).'/local/crontab/'.$fileInfo['filename'].'.'.$fileInfo['extension']);
    @unlink($filename);
} */
